...
`forbidden_functions` still matches on the bare function id and applies in every namespace. A denial
that held in one namespace and not another is a denial nobody can reason about.

`allowed_functions` no longer does. It now applies in `default` only. A grant issued with no
namespace used to answer in every namespace, so a grant for one tenant answered for every other
tenant exposing the same function id. If your auth function returns `allowed_functions` for a worker
that runs in a namespace, move those grants into `namespaces`, which the auth result now carries:

```json
{
  "namespaces": { "orders": ["svc::*"] }
}
```

The keys are also the namespaces that session may declare on `engine::workers::register`, so an auth
function that scopes grants also scopes where the worker may register. Leave `namespaces` empty to
keep the previous behaviour: the session declares any namespace, and only `allowed_functions` and
`expose_functions` apply. See
