...
`config:` block once, as a first-boot seed, and its runtime settings live in the
[configuration worker](../using-iii/configuration) from then on — updated live, without an engine
reload.
