...
A browser worker connects through `worker-manager`'s RBAC-gated listener, separate from the
...
{/* TODO(validation): `iii worker add worker-manager` is being added to the registry; re-verify once the command works end-to-end. */}

## Run two listeners

The engine's built-in port at `49134` is the **trusted** listener; local workers (link worker,
analytics worker) connect there. The browser must not. Add two `worker-manager` entries: the
...
names a function `worker-manager` invokes on every connection to admit or reject it; you write
that next.
...
format and port, this goes back to the setup we did with `worker-manager` and RBAC.
...
wrapper `stream` delivers to subscribers.
...
RBAC-gated listener (via `worker-manager`) that uses an auth function to admit it because the
