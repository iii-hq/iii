...
Add `auth` to the root `worker-compose.yaml` catalog and reference it from the tutorial stack with
`catalog://auth`. The descriptor declares its package manifest, exact build toolchain, bundle
files, and runtime argv.
...
worker) connect there. The browser must not. Stop the engine and Compose, then add two
`iii-worker-manager` entries to `config.yaml`: the trusted one and an RBAC-gated instance on `3110`
for browsers:
...
workers:
  ...
  - name: "iii-worker-manager#browser"
    config:
      host: 127.0.0.1
      port: 3110
      rbac:
...
Restart the engine with its configuration, then reconnect Compose to that engine:
...
The Compose daemon exposes editing `worker-compose.yaml`. Target the daemon serving the `linkly` namespace:
