...
durable execution, storage, analytics, expiry, a live ops dashboard, and an autonomous link-safety
agent.
...
Drop in the `harness` agent to sample new links, investigate suspicious destinations in an
`iii-sandbox`, and decide on its own whether to quarantine them or ask a human to confirm deletion.
