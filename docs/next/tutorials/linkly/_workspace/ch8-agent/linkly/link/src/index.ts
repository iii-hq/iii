...
// Store a redirect rule for a code. `rule` is a Node.js script body that
// receives the request as JSON in argv[2] and writes JSON with `chosen_url`.
...
// Run a redirect rule in an iii-sandbox: untrusted user code runs in an
// ephemeral microVM, never on the link process. Returns the chosen URL,
// or null on any failure (which falls back to the default `url`).
  ...
  try {
    ...
    >({
      ...
      payload: {
        ...
        cmd: 'node',
        args: ['-e', rule],
        // Pipe the request to stdin so the rule reads it positionally-free.
...
worker.registerFunction('link::resolve', async (payload: { code: string }) => {
  // Quarantined links don't resolve, even if cached. This is the agent's
  // auto-action: a malicious link goes dark immediately.
...
worker.registerFunction('link::quarantine', async (payload: { code: string; reason: string }) => {
  ...
  // Evict the cache so resolves go through the quarantine check.
  await worker.trigger({ function_id: 'state::delete', payload: { scope: 'links', key: payload.code } })
...
worker.registerFunction('http::redirect', async (req) => {
  ...
  // If a redirect rule is attached, run it in a sandbox; it may pick a target.
