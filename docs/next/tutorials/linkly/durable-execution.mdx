...
don't have link updating functionality just yet so we'll add that and an HTTP endpoint for it too.
...
worker.registerFunction("link::update", async (payload: { code: string; url: string }) => {
  ...
  await worker.trigger({
    ...
    payload: { topic: "link.updated", data: { code: payload.code, url: payload.url } },
  });
...
from a dedicated `click-streamer` worker and bulk-load links from a CSV over a channel.
