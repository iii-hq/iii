...
<Note>
  `iii-queue` has been running since Chapter 1, so its `config.yaml` block was already consumed as a
  first-boot seed — its settings now live in `./config/iii-queue.yaml`. If the block above is no
  longer in your `config.yaml`, make the same change under `value:` in that file instead; it applies
  live, no restart needed. See [Configuration](/using-iii/configuration).
</Note>
