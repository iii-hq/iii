...
that: a `link::create` and a `link::resolve` function, exposed over HTTP. The engine already runs
the HTTP server and the key-value store, so the agent writes only these two functions, and you skip
...
When a redirect misbehaves you need to see why. This chapter adds no worker: the engine already
traced every redirect, so here you only read what it recorded, the span tree and the logs for each
request.
...
Open another tab with the **+**, then select the **Logs** page, and read the structured log lines.
Each row's trace ID pivots straight to its trace.
...
codes are taken, so it reports `{ imported: 0, skipped: 2 }` rather than failing. Confirm the links in
the console at [http://127.0.0.1:3113](http://127.0.0.1:3113).
