...
`analytics` in Chapter 4. `iii-stream` must start with the engine, so declare it in `config.yaml`:
...
Restart the engine and reconnect the tutorial stack:
...
Add `click-streamer` to the root `worker-compose.yaml` catalog and reference it from the tutorial
stack with `catalog://click-streamer`. The descriptor declares its package manifest, exact build
toolchain, bundle files, and runtime argv.
