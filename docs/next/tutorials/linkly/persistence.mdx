...
<Note>
  If the `database` worker already booted with its default settings, the `config.yaml` block has
  been consumed as a seed (and may have been replaced with a breadcrumb comment). In that case make
  the same change under `value:` in `./config/database.yaml` — it applies live. See
  [Configuration](/using-iii/configuration).
</Note>
