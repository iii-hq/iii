...
<Info>
  If you prefer to keep `iii-state` as the only persistence layer rather than introducing a
  database, change it to file-backed instead by setting `store_method: file_based` and a `file_path`
  in the `iii-state` entry in `config.yaml`. The rest of this chapter assumes the database route.
</Info>
...
cache for the next read. It's easiest to just replace the existing `link::resolve` function with our
new version:
...
Since we have a database now it's trivial to start click tracking. Pull the write into its own
