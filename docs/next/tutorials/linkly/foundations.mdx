...
<Note>
  A worker's `config.yaml` block seeds its settings on that worker's first boot. After that boot,
  settings live in one file per worker under `./config/` and are edited there or from the console —
  a later `config.yaml` edit won't apply. See [Configuration](/using-iii/configuration).
</Note>
