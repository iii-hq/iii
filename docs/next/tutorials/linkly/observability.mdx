...
`iii project init` added the `observability` worker to `config.yaml`, and from then on every
...
<Info>
  **observability emits OpenTelemetry.** Its traces, metrics, and logs are emitted as OTel, so
  ...
  [workers.iii.dev/workers/observability](https://workers.iii.dev/workers/observability);
