...
Add `bulk-importer` to the root `worker-compose.yaml` catalog and reference it from the tutorial
stack with `catalog://bulk-importer`. The descriptor declares its package manifest, exact Node and
package-manager versions, build argv, bundle files, and runtime argv.
