...
<Warning>
  The iii `harness` has a bug on installation that is currently being fixed. This chapter is up so
  you can see what the workflow is like. The chapter will be brought to working condition shortly.
</Warning>
