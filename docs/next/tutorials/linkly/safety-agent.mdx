...
// Pull the link's URL out of the first user message, and the latest tool_result
// (an inspect_url response) out of any later ones.
...
// Build a tool_use block in the same shape the provider returns.
...
`link-safety-agent/src/index.ts` defines the worker. It subscribes to `link.created`, samples at the
