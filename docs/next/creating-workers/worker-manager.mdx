...
description:
  "Accept untrusted worker connections through RBAC-gated listeners served by the worker-manager
  worker."
owner: "devrel"
type: "how-to"
---

The `worker-manager` worker owns the connections between workers and the engine. It always runs
...
<Note>
  `worker-manager` is always running, so there is nothing to add; you configure it directly in
  `config.yaml`. This page is a quick tour of RBAC listeners. For the full configuration see the
  [worker-manager worker docs](https://workers.iii.dev/workers/worker-manager).
</Note>
...
<Warning>
  Declaring **any** `worker-manager` in `config.yaml` replaces the default `49134`
...
<Warning>
  ...
  is defined and managed by the trusted `worker-manager` and is not part of a payload sent by
  the worker.
</Warning>
...
<Note>
  ...
  separating dissimilar workers to separate `worker-manager` definitions. You can define any
  number of `worker-manager`s in iii's configuration.
</Note>
...
<Tabs>
  ...
  <Tab title="Rust">
    ```rust src/main.rs
    use iii_helpers::worker_connection_manager::AuthInput;
    ...
    worker.register_function("auth::browser", RegisterFunction::new(|input: AuthInput| {
        ...
        if token.map(String::as_str) != Some(expected.as_str()) {
            return Err(iii_sdk::Error::Handler("unauthorized".into()));
        }
        Ok(json!({
            "allowed_functions": [],          // extra IDs to allow beyond expose_functions
            "forbidden_functions": [],        // denied even if matched; supersedes allowed
