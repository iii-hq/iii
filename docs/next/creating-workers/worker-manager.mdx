...
Add a second `iii-worker-manager` entry to `config.yaml` for the untrusted port (here `3111`, the
port the browser SDK connects to by default):

```yaml config.yaml
workers:
  # ...
  # Browser-facing, RBAC-gated listener. The trusted 49134 listener stays as is.
...
`auth_function_id` names a function the worker invokes once per connection to authenticate the
worker to connect (or reject it if authentication fails). That is covered in the next section.
...
worker.registerFunction(
  ...
  async (input: {
    ...
    return {
      ...
      forbidden_functions: [], // denied even if matched; wins over allowed
      allow_trigger_type_registration: false,
      allow_function_registration: true, // the browser registers its own callbacks
...
the untrusted client connects through `3111`.
...
export const worker = registerWorker(`ws://localhost:3111?token=${encodeURIComponent(token)}`);
```
