...
<Warning>
  Declaring **any** `iii-worker-manager` in `config.yaml` replaces the engine's built-in `49134`
...
workers:
  # ...
  # Trusted listener for your own workers. Replaces the engine's built-in 49134.
...
<Tabs>
  ...
  <Tab title="Rust">
    ```rust src/main.rs
    use iii_sdk::{AuthInput, InitOptions, RegisterFunction, register_worker};
    use serde_json::json;
