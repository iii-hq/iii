---
title: "Define new trigger types in an iii worker"
description: "Bind functions to existing trigger types like http, cron, queue, and state, or register a new trigger type so other workers can react to events you emit."
owner: "devrel"
type: "how-to"
---
...
[`state` changes](https://workers.iii.dev/workers/state), and any other event source in the system.
...
Most workers consume trigger types that other workers already publish. Bind one of your worker's
functions to a trigger type with `worker.registerTrigger({ type, function_id, config })`.

<Info>
  You can register against a trigger type before its publishing worker is connected. The engine
  stores the binding optimistically and activates it automatically once that worker joins. See
  [Registering before a trigger type is
  available](../using-iii/triggers#registering-before-a-trigger-type-is-available) for how
  optimistic registrations behave.
</Info>
...
<Note>
  Trigger _types_ have no metadata field of their own; metadata is attached per binding.
...
The example below sketches a tiny version of [http](https://workers.iii.dev/workers/http), the
worker that publishes the real `http` trigger type. The publisher worker:
