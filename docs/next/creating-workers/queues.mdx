...
The example uses the standard delivery mode with three attempts and up to ten jobs in flight. See
the [queue worker configuration reference](https://workers.iii.dev/workers/queue) for every
accepted field, default, and FIFO option.
