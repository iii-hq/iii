...
To copy a file between the host and a running sandbox, use the `iii worker sandbox upload` and
`iii worker sandbox download` admin commands. These are current CLI commands with no `sandbox::*`
trigger equivalent: they stream raw bytes through an iii data channel, so transfers have no
JSON-envelope size cap. Every other sandbox operation on this page goes through `iii trigger
sandbox::*`; file transfer alone keeps the CLI form:
