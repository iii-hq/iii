...
serving network traffic is not reaped while it is handling requests. Bare acknowledgements and
keepalive probes do not count as activity. Each exec remains bounded by `max_exec_timeout_ms`, and
you should still call `sandbox::stop` when the sandbox is no longer needed.
