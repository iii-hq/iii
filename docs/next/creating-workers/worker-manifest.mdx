...
description: "Field reference for iii.worker.yaml: name, dependencies, runtime base image, setup and start scripts, env, resources, and publish metadata."
owner: "devrel"
type: "reference"
---
...
license: Apache-2.0 # SPDX license id the registry publishes the worker under
...
`iii`, `deploy`, `manifest`, and `license` are strings. `license` is the SPDX identifier the
workers registry publishes the worker under and is required by the workers-repository release CI.
`tags` is a list of strings used for agent and user discovery in the workers registry. Keep tags
short, lowercase, and focused on terms someone would search for rather than repeating the worker
description.
