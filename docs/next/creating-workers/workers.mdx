...
<Note>
  ...
  client or a third party's), connect through the `worker-manager` RBAC listener and gate it
  with an auth function instead. See [Untrusted workers and access
  control](../using-iii/workers#untrusted-workers-and-access-control) and the [worker-manager
  worker page](https://workers.iii.dev/workers/worker-manager).
</Note>
