...
`III_NAMESPACE` environment variable, and falls back to the engine's `default` namespace when neither
is set. The browser SDK has no environment to read, so it takes the option only.

<Note>
  For calling across namespaces and handling a rejected registration, see [Isolate workers with
  namespaces](../how-to/isolate-workers-with-namespaces).
</Note>
