...
<Tabs>
  <Tab title="Node / TypeScript">
    ...
    const worker = registerWorker(url, {
      ...
      namespace: process.env.III_NAMESPACE, // scopes this worker's registrations
    });
    ```
  ...
  <Tab title="Python">
    ...
    worker = register_worker(
        ...
        InitOptions(
            ...
            namespace=os.environ.get("III_NAMESPACE"),  # scopes this worker's registrations
        ),
    )
    ```
  ...
  <Tab title="Rust">
    ...
    let worker = register_worker(
        ...
        InitOptions {
            ...
            namespace: std::env::var("III_NAMESPACE").ok(), // scopes this worker's registrations
            ..Default::default()
        },
    );
    ```
...
can coexist in another namespace. The SDK falls back to the `default` namespace when it is not set.
The browser SDK has no environment to read, so it takes the option only.

Because the option is a environment variable and not a literal, one worker package serves many
tenants: read the namespace from the environment, as the examples do, and each deployment of the
same image registers in its own namespace.
