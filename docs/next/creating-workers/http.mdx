...
The `http` worker turns your functions into REST routes exposed as HTTP endpoints. It is the
engine's inbound HTTP server worker; `iii-http-functions` separately handles outbound HTTP calls
from functions.
...
<Tabs>
  ...
  <Tab title="Rust">
    ...
    let worker = register_worker(&url, InitOptions {
