...
| `engine::registered-triggers::list` | List every registered trigger instance. Filter by `include_internal`, `trigger_type`, `function_id`, `namespace`, `worker`, or `search`. Each row carries a `status`; pass `include_pending: true` to also list registrations with `status: pending`, whose trigger type has no provider yet. |
