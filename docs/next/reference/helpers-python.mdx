...
| `namespaces` | `dict[str, list[str]]` | No | Grants scoped to one namespace each, e.g. ``\{"orders": ["svc::*"]\}``. The keys are also the namespaces the session may declare on ``engine::workers::register``. Leave empty to scope nothing. |
