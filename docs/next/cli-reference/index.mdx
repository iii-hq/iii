...
| ------ | ----------- |
| `--no-wait` | Return immediately. Don't block waiting for the worker to report ready |
| `--port <PORT>` | Engine WebSocket port the spawned worker connects back to. Defaults to DEFAULT_PORT; the engine passes its configured worker-manager port when auto-spawning external workers so non-default manager ports don't silently break connectivity [default: 49134] |
