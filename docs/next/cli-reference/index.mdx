...
| `-f, --file <PATH>` | Path to the worker-compose.yaml this invocation is bound to |
| `--up` | Bring the project up as soon as the daemon connects, then keep serving. The daemon exits with a non-zero code if that first `up` fails, which is what makes compose usable from a script or a CI job |
