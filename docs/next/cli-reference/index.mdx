...
| [`worker`](#iii-worker) | Install and manage immutable Registry worker packages. |
...
| Command | Description |
| ------- | ----------- |
| [`add`](#iii-worker-add) | Install an immutable worker package from the iii registry. To run local source, declare it in worker-compose.yaml and use `iii compose up`. By default `add` waits up to 120s for the worker to report ready. After which the worker will continue to boot but the command will return to the shell. See `iii worker status` to continue observing a booting worker and `iii worker logs` for logs |
...
Install an immutable worker package from the iii registry. To run local source, declare it in worker-compose.yaml and use `iii compose up`. By default `add` waits up to 120s for the worker to report ready. After which the worker will continue to boot but the command will return to the shell. See `iii worker status` to continue observing a booting worker and `iii worker logs` for logs
...
| Argument | Description |
| -------- | ----------- |
| `<WORKER[@VERSION]>...` | iii registry package references (ex. `database` or `pdfkit@1.0.0`) |
