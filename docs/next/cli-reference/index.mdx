...
| [`console`](#iii-console) | Launch the iii web console. Dispatches to the `iii-console` binary; see the linked section for its full surface. |
...
| [`worker`](#iii-worker) | Manage workers (add, remove, list, info). Dispatches to the `iii-worker` binary; see the linked section for its full surface. |
...
| [`create`](#iii-worker-sandbox-create) | Create a long-lived sandbox and print its id to stdout. Pair with `iii worker sandbox exec <id>` and `iii worker sandbox stop <id>` |
