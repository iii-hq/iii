---
title: "Install the iii engine"
description: "Install the iii engine on Linux, macOS, or Windows (via WSL2), verify the version, and prepare your local development environment."
owner: "devrel"
type: "how-to"
---
...
<Note title="Windows">
  The install script targets Linux and macOS. On Windows, run it from a
  [WSL2](https://learn.microsoft.com/windows/wsl/install) shell:

  ```powershell
  wsl --install
  ```

  Then open the WSL2 terminal and run the `curl … | sh` command above. A native Windows installer
