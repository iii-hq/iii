...
from the terminal with the command `iii trigger`:
...
[`worker.trigger`](./triggers#call-a-function-directly)). The only thing special about them is that
you didn't have to register them.
