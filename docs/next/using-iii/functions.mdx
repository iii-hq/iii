...
event-source workers like http, cron, queue, state, and stream. All paths leave
...
- **Stream**: Real-time push to connected clients over WebSocket. See
  [stream](https://workers.iii.dev/workers/stream).
...
- **Observability**: Traces, logs, metrics, alerts, sampling rules, and rollups. See
  [observability](https://workers.iii.dev/workers/observability).
