...
<Note>
  Functions can also run when a trigger is satisfied — an `http` request, a `cron` schedule, a
  `state` change, and so on. To bind a function to an event source, see [Triggers / Register a
  trigger](./triggers#register-a-trigger).
</Note>
