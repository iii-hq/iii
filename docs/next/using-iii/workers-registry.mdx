...
`config.yaml` and started automatically. Its settings are seeded from `config.yaml` on first boot
and managed through the [configuration worker](./configuration) afterwards.
