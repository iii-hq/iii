...
the call resolves in `default`. The caller's own namespace does not select the target namespace.
...
<Warning>
  `iii trigger` has no namespace option. It calls functions in `default` only. Use an SDK to call a
