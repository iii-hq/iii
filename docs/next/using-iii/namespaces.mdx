...
<Warning>
  A worker's namespace is not inherited into `worker.trigger()` and `worker.registerTrigger()`
  calls.
</Warning>
...
namespace of the worker that calls it: a binding with no `namespace` field targets `default`. For
example, a worker in `orders` that registers a trigger without the field gets a trigger that targets
the `default` namespace instead of its own function.
