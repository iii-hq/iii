...
## Enqueue functions in a namespace

`TriggerAction.Enqueue` resolves the `engine::queue::enqueue` provider in the target function's
namespace. A namespace whose functions are enqueued needs its own namespace-aware queue worker; the
engine fails the enqueue with `enqueue_error` rather than falling back to a provider in `default`.
See [Queue providers and namespaces](../creating-workers/queues#queue-providers-and-namespaces).
