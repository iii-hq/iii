---
title: "Workers"
description: "Add and operate workers with iii worker or Compose."
owner: "devrel"
type: "how-to"
---
...
## Manage workers with iii worker

`iii worker` manages standalone worker packages through the public `iii.worker.yaml` contract. It
accepts Registry names, OCI references, and local worker directories:

```bash
iii worker add state
iii worker add ./workers/my-worker
iii worker add ghcr.io/example/my-worker:latest
iii worker list
iii worker status state
```

`iii worker init` scaffolds a local worker manifest. Add, remove, reinstall, update, clear, start,
stop, restart, sync, status, logs, and exec remain available. Registry dependency resolution is
pinned in `iii.lock`; local and bundled workers keep their manifest-defined runtime behavior.

Use Compose for declarative multi-worker stacks, dependency startup order, or release descriptor
authoring. The two surfaces coexist; adopting Compose does not delete a worker's public manifest.
...
Existing `iii worker` projects remain supported. To move project orchestration into a declarative
stack, follow the [Compose catalog guide](../upgrading/workers-to-compose); keep each
`iii.worker.yaml` for public development and package compatibility.
