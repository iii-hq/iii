...
unrestricted access. For those, connect them through the `worker-manager` worker, which exposes
...
[worker-manager worker page](https://workers.iii.dev/workers/worker-manager).
