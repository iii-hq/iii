...
Write a `Dockerfile` and a `docker-compose.yml` at the project root, then bring the stack up with
Compose:
...
A `docker-compose.yml` for the engine exposes:
...
Build the image from `iiidev/iii:latest` (distroless, non-root). Add Redis or RabbitMQ services to
the compose file when workers need those external adapters.
