...
Commands such as `iii worker` and `iii console` run separate `iii-*` binaries. iii looks for those
binaries on `PATH` first, then falls back to its managed install directory (`~/.local/bin` on macOS
and Linux, `%APPDATA%\iii\bin` on Windows). This lets a development or override build on `PATH`
take precedence. `iii update` still updates the managed copy, even while a different copy is being
selected from `PATH` for execution.
