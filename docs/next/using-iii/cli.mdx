...
| --- | --- |
| `iii compose` | Start the Compose daemon without an initial project. This is an intentional verbless invocation. |
| `iii compose up` | Valid nested subcommand that supervises a declared engine and project workers. |
...
`iii trigger -n <namespace> <function>` is the short form of
`iii trigger --namespace <namespace> <function>`; both forms are valid.
