...
<Note>
  ...
  traces, logs, and metrics from the observability worker. For scripting or agent integration,
...
<Info title="Observability worker required">
  Trace collection requires the observability worker with traces enabled. The Traces page reads
...
<Info title="Observability worker required">
  Log collection requires the observability worker with logs enabled. Use `logs_exporter:
