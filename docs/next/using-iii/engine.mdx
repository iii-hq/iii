...
[configuration worker](./configuration); from then on its settings live in one file per worker
under `./config/` — editable in real time from disk, the console, or `configuration::set` — and the
engine strips the consumed block from `config.yaml`, leaving a breadcrumb comment in its place. See
