...
| -------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------- |
| `compose::up`        | `file`, `container`                                         | A concise [mutation outcome](#mutation-outcomes).                          |
...
A call that names no workers reconciles the file in place and returns a
[mutation outcome](#mutation-outcomes) directly.
...
version is specified or to the target version when it is. On the CLI, repeat `worker=` for each
worker; a JSON payload can use a single `workers` list.
...
| -------------- | ------------------------------------------------------------------ |
| `file`         | The project to edit.                                              |
| `workers`      | The canonical JSON list of worker specs to update.                |
| `worker`       | One worker spec: `name` or `name@version`. Repeatable on the CLI. |
| `operation_id` | A caller-selected id for the [operation](#observable-operations). |
...
| Daemon and project  | `NO_COMPOSE_FILE`, `WRONG_DAEMON`, `INVALID_NAMESPACE`, `UNKNOWN_CONTAINER`, `UNKNOWN_PROJECT`, `INVALID_STATE_FILE`, `STATE_DIR_UNAVAILABLE`, `DAEMON_ALREADY_SERVING`, `DAEMON_NAMESPACE_TAKEN`, `OPERATION_CANCELLED`, `OPERATION_ID_ALREADY_EXISTS`, `IO_ERROR`                                                                                             |
