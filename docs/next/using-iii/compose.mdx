...
conflicting backup is never overwritten: while the destination exists it marks a source
rewritten after adoption (for example, a worker that still registers the bare id), which is
left in place with no writes; without a destination it stops migration with an error.
