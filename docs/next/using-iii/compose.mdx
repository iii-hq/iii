...
Workers ship with scripts specified in `iii.worker.yaml` and they can be overwritten with scripts in
a project's `worker-compose.yaml`. If both sources are missing a `run` command then the worker fails
with `MISSING_START_COMMAND`.
