...
title: "Compose"
description:
  "The compose daemon's compose::* functions with mutation outcomes and observable operations, the
  worker-compose.yaml schema, and the worker environment."
owner: "devrel"
type: "how-to"
---
...
| -------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------- |
| `compose::up`        | `file`, `container`, `frozen`                               | A concise [mutation outcome](#mutation-outcomes).                          |
| `compose::down`      | `file`, `container`                                         | A concise [mutation outcome](#mutation-outcomes).                          |
...
| `compose::add`       | `file`, `workers`, `operation_id`                           | An [accepted operation](#observable-operations): its id and worker count.  |
| `compose::remove`    | `file`, `workers`, `operation_id`                           | An [accepted operation](#observable-operations): its id and worker count.  |
| `compose::restart`   | `file`, `container`                                         | A concise [mutation outcome](#mutation-outcomes).                          |
| `compose::update`    | `file`, `workers`, `operation_id`                           | An [accepted operation](#observable-operations): its id and worker count.  |
| `compose::operation` | `operation_id`                                              | An operation's latest snapshot: its status, phase, and progress.           |
| `compose::cancel`    | `operation_id`                                              | The operation id and whether cancellation was accepted.                    |
...
iii trigger compose::operation --namespace dev operation_id=deploy-42
iii trigger compose::cancel  --namespace dev operation_id=deploy-42
...
`compose::up`, `compose::down`, and `compose::restart` return one concise outcome rather than the
full reconciliation state. Full worker state stays available through
[`compose::status`](#checking-status), and process output through [`compose::logs`](#viewing-logs).

| Field                   | Description                                                                    |
| ----------------------- | ------------------------------------------------------------------------------ |
| `status`                | `ok` or `failed`.                                                              |
| `changed`               | `true` when the call changed anything.                                         |
| `worker`                | The primary worker a targeted mutation named. Absent for whole-project calls.  |
| `workers`               | Every explicitly requested worker, present when more than one was supplied.    |
| `version`               | The resolved package version, when the mutation resolves one.                  |
| `affected_workers`      | Other workers the mutation had to change, such as restarted dependents.        |
| `error`                 | The first worker failure, as a stable `code` and a sanitized `message`.        |
| `not_required_failures` | Workers that failed while the operation still succeeded, which is only possible for a container whose effective `required` value is `false`. |

Optional fields are omitted from the response when they do not apply.

### Observable operations

`compose::add`, `compose::remove`, and `compose::update` do not wait for reconciliation. A call
that names workers is admitted immediately and returns an acceptance:

```json
{ "operation_id": "compose:6f0c33f0-…", "status": "accepted", "requested": 2 }
```

The mutation then runs in the background under that `operation_id`. Pass your own `operation_id`
in the request to choose the id yourself. A caller-selected id lets you bind a progress trigger
before submitting the mutation, so no event can be missed. An id that is already in use is refused
with `OPERATION_ID_ALREADY_EXISTS`.

An add or remove that names no workers reconciles the file in place and returns a
[mutation outcome](#mutation-outcomes) directly. A bare update is admitted like any other
operation, because it updates every declared package worker.

#### Completion events

The daemon publishes operation progress through the `compose-operation` trigger type. Bind a
function to it to receive events. The trigger config takes `operation_id`, which you omit to
receive every operation from the daemon, and `terminal_only`, which delivers only the final
completion event.

```typescript
worker.registerTrigger({
  type: "compose-operation",
  function_id: "deploy::on_complete",
  config: { operation_id: "deploy-42", terminal_only: true },
});
```

Each event holds `sequence`, `operation_id`, `phase`, `detail`, `elapsed_ms`, and `terminal`, plus
`container`, `current`, and `total` when they apply. Every operation ends with exactly one terminal
...
reconnect and recovery: `operation_id`, `status` (`running`, `succeeded`, `failed`, or
`cancelled`), `phase`, `requested`, `total`, `completed`, `last_sequence`, and `last_event`.

`compose::cancel` takes `operation_id`, requests cancellation, and returns the id with a
`cancelled` flag. A cancelled mutation finishes with status `cancelled` and reports
`OPERATION_CANCELLED`.

```bash
iii trigger compose::add       --namespace dev worker=database worker=web operation_id=deploy-42
iii trigger compose::operation --namespace dev operation_id=deploy-42
iii trigger compose::cancel    --namespace dev operation_id=deploy-42
```
...
| `operation_id` | A caller-selected id for the [operation](#observable-operations). |

The call returns as soon as the operation is accepted. Follow it through
[completion events](#completion-events) or [`compose::operation`](#checking-and-cancelling-an-operation).
...
An add keeps the pins of dependencies that are already declared in the file, even when the registry
resolves a newer version for them. Only the workers you name in the call move their pins; use
[`compose::update`](#updating-workers) to move the rest. An add plans against a snapshot of the
compose file and only edits the file it planned against. A file that keeps changing while the add
resolves its graph fails the call with `ADD_PLAN_STALE` and writes nothing; retry the add.
...
`compose::remove worker=state` removes the named workers and every reference to them. The changes
are validated before a worker is removed. Compose then stops only those workers and runs an
idempotent `up`. On the CLI, repeat `worker=` for each worker; a JSON payload can use a single
`workers` list.

| Field          | Description                                                       |
| -------------- | ------------------------------------------------------------------ |
| `file`         | The project to edit.                                              |
| `workers`      | The canonical JSON list of declared worker keys to remove.        |
| `worker`       | One worker. Repeatable on the CLI.                                |
| `operation_id` | A caller-selected id for the [operation](#observable-operations). |
...
The call returns as soon as the operation is accepted, whether it names workers or updates every
declared package worker. Follow it through [completion events](#completion-events) or
[`compose::operation`](#checking-and-cancelling-an-operation).
...
`compose::stop` takes no target. A request that names a `container` or `worker` is refused with
`STOP_TAKES_NO_CONTAINER` instead of stopping the whole daemon. Use
`compose::down container=<name>` to stop one container.
...
| Daemon and project  | `NO_COMPOSE_FILE`, `WRONG_DAEMON`, `INVALID_NAMESPACE`, `UNKNOWN_CONTAINER`, `UNKNOWN_PROJECT`, `INVALID_STATE_FILE`, `STATE_DIR_UNAVAILABLE`, `DAEMON_ALREADY_SERVING`, `DAEMON_NAMESPACE_TAKEN`, `OPERATION_CANCELLED`, `OPERATION_ID_ALREADY_EXISTS`, `ADD_PLAN_STALE`, `STOP_TAKES_NO_CONTAINER`, `IO_ERROR`                                                                                                                                                   |
