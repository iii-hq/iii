...
| Command | Description |
| ------- | ----------- |
| [`add`](#iii-worker-add) | Install an immutable worker package from the iii Registry. Use `iii compose` for local source. By default `add` waits up to 120s for the worker to report ready |
...
Install an immutable worker package from the iii Registry. To run local source, declare it in the root `worker-compose.yaml` and use `iii compose up`. By default `add` waits up to 120s for the worker to report ready
...
| Argument | Description |
| -------- | ----------- |
| `<WORKER[@VERSION]>...` | iii Registry package references (for example `database` or `pdfkit@1.0.0`) |
