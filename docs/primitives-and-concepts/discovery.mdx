...
With a central registry topology changes are automatic. This is what iii calls **live discovery** — functions and triggers exposed by one worker become visible and usable across the system in real time. Combined with **live extensibility** (add new workers without redesigning the architecture) and **live observability** (observe the entire connected stack in real time), the system stays coherent as it grows.
