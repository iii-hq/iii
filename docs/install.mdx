...
inside your iii system. `provider-openai` and `provider-anthropic` are enabled by default; add
any other provider with `iii trigger compose::add` once the project is running.
