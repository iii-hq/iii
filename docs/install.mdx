...
{/\* TODO (community feedback): Add a direct binary-download path alongside the curl|sh installer so
users who do not want to verify a remote script with a bot, or are on Windows, have an alternative.
Link to the GitHub Releases page for the engine and include PATH setup instructions per OS
(especially Windows). Original feedback:

"Hey guys, sorry for writing in here, but i'd much prefer to not verify with a bot. Suggestion: add
a link to the 'install' page to the binaries/exe files found in /release on github, as well as
instructions on how to add to path, its very non-windows friendly as it is. Of course I could be
wrong and everything, so regardless: Thanks for developing III, it seems like a great tool, i'm
looking forward to see how it develops in the future." \*/}
...
{/\* TODO: re-enable the "## 3. Install the VS Code Extension (Optional)" section below once the
iii-lsp extension is more thoroughly tested across VS Code, Cursor, Windsurf, and VSCodium. The
Frame demo also needs `/images/lsp.mp4` to be captured and committed before the section is re-added.
...
<Tabs>
  ...
  <Tab title="Cursor">
    cursor --install-extension iii-hq.iii-lsp
  </Tab>
  <Tab title="Windsurf">
    windsurf --install-extension iii-hq.iii-lsp
  </Tab>
  <Tab title="VSCodium">
    codium --install-extension iii-hq.iii-lsp
  </Tab>
</Tabs>
*/}
