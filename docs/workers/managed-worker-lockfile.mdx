...
| `iii worker update [worker]` | Yes | Re-resolves `latest` for one locked worker, or all locked workers when no name is provided, then rewrites `iii.lock`. |
...
| `iii worker verify` | No | Checks that every worker in `config.yaml` has an entry in `iii.lock`. Extra lockfile entries are allowed. |
