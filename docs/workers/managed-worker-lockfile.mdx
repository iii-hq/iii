...
| `iii worker verify` | No | Checks that every lockfile-managed worker in `config.yaml` has an entry in `iii.lock`. Built-ins, direct OCI refs, and local-path sandbox workers are skipped. Extra lockfile entries are allowed. |
