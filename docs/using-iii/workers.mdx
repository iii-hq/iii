...
The worker is added to `config.yaml` and started automatically. For local paths, Docker / OCI
images, and version pinning, see [Worker Registry / Adding a worker](./registry#adding-a-worker).
...
Downloaded artifacts remain on disk after removal. See [CLI](/using-iii/cli#worker) for the
subcommand that cleans up artifacts.
