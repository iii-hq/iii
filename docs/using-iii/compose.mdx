...
| `runtime` | `host` or `vm`. Valid only for registry packages whose resolved artifact is a bundle; omitted bundles use `vm`. |
| `scripts` | `pre_run`, `pre_run_timeout`, `run`, and `post_run`. For packages, `run` requires `runtime: host` and is valid only when the resolved artifact is a bundle. |
...
containers:
  ...
  math:
    ...
    runtime: host
    scripts:
      run: .venv/bin/python src/custom_math_worker.py
  state:
...
| `runtime`         | `host` or `vm` | absent               | Runtime for registry bundle artifacts. An omitted value resolves to `vm`.                                  |
...
| `bundle` | In a VM by default. Set `runtime: host` to run it as a host process from a private persistent workspace.     |
| `path`   | A path to a worker stored locally on disk.                                                                   |

`runtime` applies only to registry packages whose resolved artifact is a bundle. Using it on a
`path://` worker, or resolving a package with `runtime` to a binary artifact, is an error. There is
no automatic fallback between VM and host: an omitted `runtime` means `vm`, and a VM start failure
does not retry the bundle on the host.
...
Registry bundles are currently refused on Windows before installation, including bundles declared
with `runtime: host`. Run Compose under WSL, where the VM has KVM to run on. Every other worker kind
runs on Windows as it always has.
...
| Field             | Type   | Default | Description                                                                                  |
| ----------------- | ------ | ------- | -------------------------------------------------------------------------------------------- |
| `pre_run`         | string | absent  | Additional Compose hook that runs to completion before installation and start.               |
| `pre_run_timeout` | string | `60s`   | Budget for `pre_run`. Rejected without a `pre_run`.                                          |
| `run`             | string | absent  | Start command for path workers, or override for a registry bundle using `runtime: host`.      |
| `post_run`        | string | absent  | Additional Compose hook that runs after the worker's exit is confirmed.                      |
...
For a bundle using `runtime: host`, Compose copies the verified bundle from the shared package cache
into a private, owner-only workspace before running any publisher script. On Unix, directories are normalized
to `0700` and files to `0600`, retaining only the owner's executable bit when needed. Insecure reused
workspaces are rebuilt before execution. Permissions are enforced again after `pre_run` and installation,
immediately before the worker is spawned. The effective order is:

```text
prepare or reuse workspace
-> scripts.pre_run from worker-compose.yaml
-> scripts.install from iii.worker.yaml, when preparation is not current
-> scripts.run from worker-compose.yaml, or scripts.start from iii.worker.yaml
-> readiness and process supervision
-> scripts.post_run from worker-compose.yaml
```

There is no `scripts.install` field in `worker-compose.yaml`. A publisher should make its install
command create isolated dependencies inside the supplied workspace, for example a Python `.venv` or
project-local JavaScript dependencies. Unsafe bundle symlinks are rejected while the private copy is
prepared. Compose reruns installation when the bundle contents, its
install command, the host OS or architecture, or the preparation format changes. A failed install is
not marked prepared. Changing only the Compose `scripts.run` override does not reinstall.
Installation uses the worker's `startup_timeout`.

For host bundles, `scripts.run` replaces the manifest's `scripts.start`; `pre_run` and `post_run`
remain additional Compose hooks. `working_dir` is ignored so publisher scripts cannot be redirected
outside the private workspace. Compose warns when it ignores that field, a manifest
`runtime.base_image`, or VM resource settings, and when `scripts.run` overrides `scripts.start`.

<Warning>
  `runtime: host` removes VM isolation. Bundle scripts run with the normal permissions of the user
  that runs the Compose daemon. Review the publisher and its scripts before selecting host runtime.
</Warning>
...
A worker's environment is defined by the following sources, from lowest to highest precedence:
...
1. A host baseline. On Unix: `PATH`, `HOME`, `USER`, `LOGNAME`, `SHELL`, `TERM`, `TMPDIR`, `TZ`,
   ...
   and `PATHEXT`.
2. For a registry bundle using `runtime: host`, the `env` map in `iii.worker.yaml`.
3. The worker's `env_file` entries, then its `environment` map.
...
| `III_ISOLATION`         | `libkrun` for VM workers or `host` for host processes.                        |

Declaring a reserved variable in `environment` or an `env_file` fails with `RESERVED_ENV_OVERRIDE`.
Reserved entries from a host bundle's manifest are ignored, so the publisher cannot override the
runtime contract.
...
variable is set. A later `compose::up` reuses a valid cached artifact if it exists. Host bundles are
copied to their project-private workspaces before execution; publisher scripts never run directly in
this global cache.
...
| `host/<container>/workspace/` | Private persistent workspace for a registry bundle using `runtime: host`.            |
| `host/<container>/.iii-compose-host-prepared.json` | Preparation marker used to decide whether `scripts.install` must run. |
...
| Worker declaration  | `MISSING_VERSION_FOR_PACKAGE`, `RUNTIME_ONLY_FOR_BUNDLES`, `PACKAGE_RUN_REQUIRES_HOST_RUNTIME`, `RUNTIME_REQUIRES_BUNDLE`, `INVALID_HOST_BUNDLE_CONTAINER_KEY`, `NOT_A_PACKAGE_CONTAINER`, `PRE_RUN_TIMEOUT_WITHOUT_PRE_RUN`, `RESERVED_ENV_OVERRIDE`, `MISSING_ENV_FILE`                                                                                                                                                                                         |
