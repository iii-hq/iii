...
<Note>
  ...
  [iii-observability worker](https://workers.iii.dev/workers/iii-observability). For scripting or
...
Selecting a worker opens a detail panel with full metadata, telemetry, live metrics (memory
...
<Info title="Observability worker required">
  ...
  [iii-observability worker](https://workers.iii.dev/workers/iii-observability) with traces
  enabled.
</Info>
...
<Info title="Observability worker required">
  ...
  [iii-observability worker](https://workers.iii.dev/workers/iii-observability) with logs enabled.
</Info>

## Configuration

The console reads engine and observability settings from CLI flags and environment variables. The
**Config** page in the sidebar surfaces the resolved values at runtime, alongside live stats
(modules, functions, triggers, streams, workers, health), endpoint status for the engine REST
API, Streams WebSocket, and SDK Bridge, and a list of the trigger types, adapters, trigger
handlers, and worker pools currently active.
...
| `--host`              | `127.0.0.1`   | Host the console binds to                         |
| `--engine-host`       | `127.0.0.1`   | Host of the iii engine                            |
| `--engine-port`       | `3111`        | Engine HTTP API port                              |
| `--ws-port`           | `3112`        | Engine Streams WebSocket port                     |
| `--bridge-port`       | `49134`       | Engine SDK Bridge WebSocket port                  |
| `--no-otel`           | `false`       | Disable the console's own OpenTelemetry export    |
| `--otel-service-name` | `iii-console` | Service name used in the console's own OTel spans |
...
| ------------------- | --------------------- |
| `OTEL_DISABLED`     | `--no-otel`           |
| `OTEL_SERVICE_NAME` | `--otel-service-name` |
| `III_ENABLE_FLOW`   | `--enable-flow`       |

### Default ports

| Service              | Port    |
| -------------------- | ------- |
| iii Engine HTTP API  | `3111`  |
| iii Engine WebSocket | `3112`  |
| iii Console UI       | `3113`  |
| iii SDK Bridge       | `49134` |
