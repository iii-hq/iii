...
<Note>
  The full surface is also published as a generated reference: [iii](../cli-reference/iii),
  [iii worker](../cli-reference/iii-worker), and [iii console](../cli-reference/iii-console).
  Those pages are regenerated from the CLI definitions whenever the CLI changes, so they always
  match the binaries.
</Note>
