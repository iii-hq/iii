...
event-source workers like iii-http, iii-cron, iii-queue, iii-state, and iii-stream — without
changing the handler.

The two most common ways to invoke a function directly are from worker code with `worker.trigger`
or from the terminal with `iii trigger`:
...
<Note>
  Both calls are synchronous by default — they wait for the function to return. For
  fire-and-forget (`TriggerAction.Void`), queue-routed delivery (`TriggerAction.Enqueue`),
  per-worker custom actions, condition gating, and binding to event-source triggers, see [Triggers
  / Call a function directly](/using-iii/triggers#call-a-function-directly).
</Note>
