...
`iii worker add` is the npm moment for systems. What installs is not a library — it is a running participant. A queue worker. A sandbox worker. A classifier. One command, complete capability, immediately available to every other worker in the system.
