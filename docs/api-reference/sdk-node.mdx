...
| `request_format` | `Record<string, unknown>` | No | JSON Schema describing expected request format. |
| `response_format` | `Record<string, unknown>` | No | JSON Schema describing expected response format. |

<Note>
The engine auto-generates standard [JSON Schema](https://json-schema.org/) for `request_format` and `response_format` from Rust types. When listing functions, these fields contain JSON Schema objects.
</Note>
...
Custom format definition for function parameters. Can be used when manually specifying schemas during registration.

<Note>
The engine auto-generates standard [JSON Schema](https://json-schema.org/) for functions registered via the `#[service]` macro. When listing functions, `request_format` and `response_format` will contain JSON Schema objects rather than `RegisterFunctionFormat`.
</Note>
