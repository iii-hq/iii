...
Format definition for function parameters. Can be used when manually specifying schemas during registration.
...
<Note>
The engine auto-generates standard [JSON Schema](https://json-schema.org/) for `request_format` and `response_format` from Rust types. When listing functions, these fields return JSON Schema objects (not `RegisterFunctionFormat`). Use `dict[str, Any]` to handle either format.
</Note>
...
| `request_format` | [`RegisterFunctionFormat`](#registerfunctionformat) \| `dict[str, Any]` \| None | No | Schema describing expected input. Accepts `RegisterFunctionFormat` or JSON Schema. |
| `response_format` | [`RegisterFunctionFormat`](#registerfunctionformat) \| `dict[str, Any]` \| None | No | Schema describing expected output. Accepts `RegisterFunctionFormat` or JSON Schema. |
