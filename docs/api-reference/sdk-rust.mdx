...
// Using raw closure (original API, still supported)
iii.register_function(
    RegisterFunctionMessage {
        id: "echo".to_string(),
        description: None,
...
| --- | --- | --- |
| 0 | ignored | none |
| 1 | `{"field": "value"}` | Entire JSON deserialized as the arg type (use `#[derive(Deserialize)]` structs) |
| 2–8 | `[val1, val2, ...]` | JSON array deserialized as positional tuple |

**Auto-generated schemas:** `request_format` and `response_format` are automatically
populated from the function's type signature using Rust type names mapped to
...
Wraps a **sync** function into an III-compatible handler. Supports 0–8 parameters.
Use with [`register_function`](#register_function) or prefer [`RegisterFunction::new`](#registerfunction) with [`register`](#register).

```rust
use iii_sdk::{iii_fn, RegisterFunctionMessage};

fn add(a: i32, b: i32) -> Result<i32, String> { Ok(a + b) }

iii.register_function(
    RegisterFunctionMessage { id: "add".into(), ..Default::default() },
    iii_fn(add),  // JSON input: [3, 4]
);
```
...
```rust
use iii_sdk::{iii_async_fn, RegisterFunctionMessage};
...
iii.register_function(
    RegisterFunctionMessage { id: "fetch".into(), ..Default::default() },
    iii_async_fn(fetch),  // JSON input: "https://..."
);
```
