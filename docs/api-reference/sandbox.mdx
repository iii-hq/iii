...
Every sandbox operation is reachable through `iii.trigger()` from worker
code or through `iii trigger sandbox::<op> …` from the terminal. There
is no longer a dedicated `iii sandbox` CLI subcommand. See
[CLI reference](#cli-reference) for the trigger forms.
...
Each `iii trigger` accepts either `key=value` tokens (shown above) or a single `--json` payload. The two forms are interchangeable: the kv form parses each value as JSON when possible and falls back to a string. The `--json` equivalent of the second exec:

```bash
iii trigger sandbox::exec --json "{\"sandbox_id\":\"$SB\",\"cmd\":\"python3\",\"args\":[\"-c\",\"import sys; print(sys.version)\"]}"
```
...
The dedicated `iii sandbox` subcommand was removed in 0.13.0. Every
sandbox operation now goes through [`iii trigger`](/how-to/trigger-functions-from-cli),
which has a single shape: a function path plus a payload (kv pairs or
`--json`). The four most useful flows are below; see the
[trigger catalog](#api-surface-at-a-glance) above for the full list.

<Warning>
`iii trigger` is request/response only. The old `iii sandbox exec`
streamed stdout/stderr line-by-line; the trigger form buffers the
whole output and returns it as part of the result. For interactive
or long-running execs, call `sandbox::exec` from SDK code where you
can attach to the streaming channel directly.
</Warning>
...
Each call takes either `key=value` tokens (shown above) or `--json '<obj>'`. The kv form parses each value as JSON when possible and falls back to a string. That is why `args='[...]'` is interpreted as an array. The `--json` equivalent of the exec call:

```bash
iii trigger sandbox::exec --json "{\"sandbox_id\":\"$SB\",\"cmd\":\"python3\",\"args\":[\"-c\",\"print(2 ** 10)\"]}"
```
...
The previous `iii sandbox upload` / `iii sandbox download` commands
streamed file bytes through an iii data channel. `iii trigger` does
not expose channels, so host↔guest byte movement from the terminal is
no longer supported. Use either:

- `sandbox::fs::write` and `sandbox::fs::read` from SDK code, where
  you can hold the channel reference (see
  [filesystem operations](#sdk-filesystem-operations)).
- `sandbox::exec` for in-VM shell tools (`tar`, `curl`, `cat`, etc.)
  when the payload size fits the envelope.
