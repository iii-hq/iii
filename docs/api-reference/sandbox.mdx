...
```bash
iii trigger sandbox::run image=python cmd='["python3","-c","print(\"hi\")"]'
```

Or, equivalently, with a single `--json` payload:

```bash
iii trigger sandbox::run --json '{"image":"python","cmd":["python3","-c","print(\"hi\")"]}'
```
