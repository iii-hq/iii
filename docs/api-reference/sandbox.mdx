...
description: "Complete reference for sandbox triggers, configuration, error codes, and CLI commands."
---

Reference for the `sandbox::*` runtime API — ephemeral microVMs spawned via `iii.trigger()`.

For procedural guidance on creating sandboxes and running commands, see [Use Sandboxes](/how-to/use-sandbox).

<Note>
This page covers the `sandbox::*` runtime API. For worker process isolation, see
[Develop Sandbox Workers](/how-to/develop-sandbox-workers).
</Note>
...
See [Engine Configuration Reference](/api-reference/engine-config) for the full
engine-level schema.
