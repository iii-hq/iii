...
| `Enqueue` | Async via named queue — returns `{ messageReceiptId }` | `trigger({ ..., action: TriggerAction.Enqueue({ queue: 'name' }) })` |
| `Void` | Fire-and-forget, no queue | `trigger({ ..., action: TriggerAction.Void() })` |
