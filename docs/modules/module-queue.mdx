...
2. **Emit events** by calling `trigger({ function_id: 'durable::publish', payload: { topic: 'order.created', data: payload } })` or `trigger({ function_id: 'durable::publish', payload: { topic, data }, action: TriggerAction.Void() })` for fire-and-forget. The `durable::publish` function routes the payload to all subscribers of that topic.
...
|---|---|---|
| **Producer** | Calls `trigger({ function_id: 'durable::publish', payload: { topic, data } })` | Calls `trigger({ function_id, payload, action: TriggerAction.Enqueue({ queue }) })` |
