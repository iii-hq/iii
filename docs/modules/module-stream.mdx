...
1. Persists the data via the configured adapter (Redis or kv)
...
The `iii-stream` worker adds three trigger types: `stream` (item changes), `stream:join` (WebSocket connect), and `stream:leave` (WebSocket disconnect).
...
Configure the `iii-stream` worker with an auth function:
