...
Enable RBAC for workers through a dedicated WebSocket port with authentication, authorization, trigger registration controls, and request interception.

```
modules::worker::WorkerModule
```

The `WorkerModule` is mandatory and always loaded. When explicitly configured in the `modules:` section, the first entry's `port` sets the main engine WebSocket port. Additional entries with `auth_function_id` start dedicated RBAC listeners. Multiple `WorkerModule` entries are supported.
...
graph LR
    ...
    RBAC -->|Invoke| Interceptor[Interceptor Function]
    Interceptor -->|engine.call| Engine[Engine]
    Engine -->|Result| RBAC
    RBAC -->|Result| Worker
```

Workers connect via WebSocket to the RBAC port. On connection, the optional auth function is called with the request headers and query parameters. Once authenticated, workers can invoke exposed functions and register triggers subject to RBAC rules. An optional interceptor function receives every invocation request before it reaches the engine.
...
<ResponseField name="port" type="number" required>
  The port to listen on. For the first `WorkerModule` entry, this sets the main engine WebSocket port. For entries with `auth_function_id`, this sets the RBAC listener port.
</ResponseField>
...
<ResponseField name="auth_function_id" type="string">
  Function ID to invoke for authentication when a worker connects. Receives headers and query parameters. Must return an `AuthResult` object. If the function fails or returns no result, the connection is rejected.
</ResponseField>

<ResponseField name="interceptor_function_id" type="string">
  Function ID to invoke before every function call from a worker. Receives a `TriggerRequest` with the target function ID, payload, and auth context. The interceptor is responsible for calling the target function and returning the result.
</ResponseField>
...
<Expandable title="AuthResult">
  ...
  <ResponseField name="allowed_trigger_types" type="string[]">
    Trigger type IDs this worker is allowed to register triggers for. Defaults to empty.
  </ResponseField>
  ...
  <ResponseField name="context" type="Record&lt;string, unknown&gt;">
    Arbitrary context object passed to the interceptor function on every invocation. Defaults to empty object.
  </ResponseField>
</Expandable>
...
3. If any `expose_functions` filter matches -- **allowed**
4. Otherwise -- **denied**

## Interceptor

When `interceptor_function_id` is configured, every worker invocation is routed through the interceptor instead of calling the target function directly.
...
The interceptor is responsible for calling the target function (e.g. via `trigger()`) and returning the result. This gives you full control to modify, validate, or audit requests.
...
- The interceptor function is the right place for request validation, rate limiting, and audit logging.
