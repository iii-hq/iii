...
<ResponseField name="logs_enabled" type="boolean">
  Whether structured log storage is enabled. When not set, log storage is always initialized by the `iii-observability` worker.
</ResponseField>
...
The `iii-observability` worker adds a new Trigger Type: `log`.
