...
The `iii-http` worker exposes registered functions as HTTP endpoints.
...
The `iii-http` worker supports a middleware system that runs functions at defined phases of the request lifecycle. Middleware functions are registered using the `http_middleware` trigger type.
