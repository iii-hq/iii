...
<Warning>
  When running multiple engine instances, the `kv` adapter does not provide reliable distributed locking — the same cron job may execute on every instance simultaneously. Use `redis` for multi-instance deployments.
</Warning>
...
The `iii-cron` worker adds a new Trigger Type: `cron`.
...
When running multiple iii Engine instances, the `iii-cron` worker uses distributed locking to ensure jobs execute only once:
