...
title: "Overview"
description:
  "A walkthrough of the four pieces that make up every iii system (workers, functions, triggers, and
  the engine), using the Quickstart scaffold as the worked example."
owner: "devrel"
type: "explanation"
---
...
A **worker** is a process that connects to the engine and announces a set of functions it can run.
Workers can run anywhere (on a laptop, in a container, in a browser tab, on a microVM) as long as
they can open a WebSocket to the engine.
...
A **trigger** is what causes a function to run. A trigger has a type (HTTP, cron, queue message,
state change, another function calling `trigger`), a configuration (which path, which schedule,
which queue), and the function ID it invokes.

The **engine** is the coordinator. It accepts worker connections, maintains a live registry of
available functions and triggers, and routes invocations to whichever worker currently provides the
requested function.
