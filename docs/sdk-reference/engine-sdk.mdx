...
The engine listens on four ports:

| Port    | Surface                                                       |
| ------- | ------------------------------------------------------------- |
| `3111`  | REST API.                                                     |
| `3112`  | Stream API (WebSocket; consumer-side stream subscriptions).   |
| `49134` | SDK WebSocket; this is what `iii_sdk::register_worker` opens. |
| `9464`  | Prometheus metrics endpoint (exposed by `iii-observability`). |
...
| `Ping` / `Pong`             | bidirectional    | Liveness; keeps idle connections from being reaped.    |
