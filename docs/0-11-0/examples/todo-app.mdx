...
<Frame caption="Todo App">
  <img src="/0-11-0/assets/todo-app.png" />
</Frame>
...
The RBAC configuration on port `3111` references `todo-project::auth-function` and explicitly lists the five functions the browser is allowed to call. See [Worker RBAC](/0-11-0/how-to/worker-rbac) for the full reference.
...
- **RBAC** — The engine's iii-worker-manager supports auth functions, expose lists, and middleware. This example uses a simple auth function that creates a session. See [Worker RBAC](/0-11-0/how-to/worker-rbac) for the full reference.
