...
<img src="/0-11-0/assets/console-workers-list.png" alt="iii Console Workers page showing connected workers grouped by runtime with status, PID, IP, and active invocation counts" />
...
<img src="/0-11-0/assets/console-workers-detail.png" alt="iii Console Workers page with a selected worker detail panel showing runtime, version, OS, metrics, and registered functions" />
...
<img src="/0-11-0/assets/console-functions-invoke.png" alt="iii Console Functions page showing grouped functions and the invoke panel for a selected function" />
...
<Info title="How-to guide">
  See [Use the Console](/0-11-0/how-to/use-console#invoke-a-function) for step-by-step instructions.
</Info>
...
<img src="/0-11-0/assets/console-triggers-http-test.png" alt="iii Console Triggers page showing HTTP triggers and the request builder for testing a selected endpoint" />
...
<Info title="How-to guide">
  See [Use the Console](/0-11-0/how-to/use-console#test-an-http-trigger) for step-by-step instructions on testing each trigger type.
</Info>

<Info title="Trigger types">
  The available trigger types depend on which workers are loaded in your engine configuration. See [Trigger Types](/0-11-0/architecture/trigger-types) and [Workers](/0-11-0/workers) for the full list.
</Info>
...
<img src="/0-11-0/assets/console-states-detail.png" alt="iii Console States page showing state groups, key-value items, and JSON value details for a selected item" />
...
<Info title="How-to guide">
  See [Use the Console](/0-11-0/how-to/use-console#manage-state-entries) for step-by-step instructions.
</Info>

<Info title="State persistence">
  State persistence depends on your engine's State worker configuration. With `in_memory` storage, state is lost on engine restart. With `file_based` or `RedisAdapter`, state persists across restarts. See [State Worker](/0-11-0/workers/iii-state) for configuration details.
</Info>
...
<Info title="Stream worker required">
  Streams are provided by the Stream worker. Make sure `iii-stream` is included in your engine configuration. See [Stream Worker](/0-11-0/workers/iii-stream) for details.
</Info>
...
<img src="/0-11-0/assets/console-queues-overview.png" alt="iii Console Queues page showing durable topics, subscriber counts, dead-letter counts, and the selected topic overview panel" />
...
<Info title="How-to guide">
  See [Use the Console](/0-11-0/how-to/use-console#send-a-test-message-to-a-queue) for step-by-step instructions on testing queues and managing dead letters.
</Info>
...
<img src="/0-11-0/assets/console-traces-waterfall.png" alt="iii Console Traces page showing a waterfall chart for a selected trace and span timing details" />

<Info title="Observability worker required">
  Trace collection requires the Observability worker with `exporter` set to `memory` or `both`. See [Observability Worker](/0-11-0/workers/iii-observability) for configuration.
</Info>
...
<img src="/0-11-0/assets/console-traces-flame-graph.png" alt="iii Console Traces page showing the flame graph view for a selected trace" />

<img src="/0-11-0/assets/console-traces-map.png" alt="iii Console Traces page showing a service trace map with connected services for a selected trace" />

<img src="/0-11-0/assets/console-traces-flow.png" alt="iii Console Traces page showing the flow graph view for a selected trace" />
...
<Info title="How-to guide">
  See [Use the Console](/0-11-0/how-to/use-console#inspect-a-trace) for step-by-step instructions on inspecting traces and spans.
</Info>
...
<img src="/0-11-0/assets/console-logs-detail.png" alt="iii Console Logs page showing structured log rows and a selected log detail panel with context data" />

<Info title="Observability worker required">
  Log collection requires the Observability worker with `logs_enabled: true`. See [Observability Worker](/0-11-0/workers/iii-observability) for configuration.
</Info>
...
<Info title="How-to guide">
  See [Use the Console](/0-11-0/how-to/use-console#navigate-the-flow-graph) for navigation instructions.
</Info>
...
<img src="/0-11-0/assets/console-config-overview.png" alt="iii Console Config page showing engine endpoints, trigger types, active workers, trigger handlers, and worker pools" />
