...
Unlike [`shutdown`](#shutdown), this method does **not** block
...
[`shutdown`](#shutdown) is used to join the thread.
...
| `registration` | `RegisterFunction` | Yes | Built via [`RegisterFunction::new`], [`RegisterFunction::new_async`], or [`RegisterFunction::http`]. Chain `.description(...)`, `.metadata(...)`, `.request_format(...)`, `.response_format(...)` as needed. |
