...
<ResponseField name="config.posthog_api_key" type="string">
  Optional PostHog project API key for sending the same anonymous product analytics events to PostHog. You can also set `POSTHOG_PROJECT_API_KEY` or `POSTHOG_API_KEY`.
</ResponseField>

<ResponseField name="config.posthog_host" type="string" default="https://us.i.posthog.com">
  Optional PostHog ingest host. Set `POSTHOG_HOST=https://eu.i.posthog.com` for EU Cloud or your self-hosted ingest URL.
</ResponseField>
