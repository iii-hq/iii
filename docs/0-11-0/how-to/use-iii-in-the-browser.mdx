...
- A running iii engine with the **iii-worker-manager** enabled and RBAC configured (see [Worker RBAC](/0-11-0/how-to/worker-rbac)).
...
<Info title="Security">
  For production, add an `auth_function_id` to validate tokens from the browser. See [Worker RBAC](/0-11-0/how-to/worker-rbac) for full auth setup.
</Info>
...
<Info title="API Reference">
  The browser SDK exposes the same `ISdk` interface as the Node SDK. See the [Browser SDK Reference](/0-11-0/api-reference/sdk-browser) for the full API.
</Info>
