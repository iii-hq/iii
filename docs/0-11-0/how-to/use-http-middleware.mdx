...
Middleware does not receive the request body. This is intentional: global middleware runs before body parsing, so auth checks and rate limiting skip the expensive JSON parse for rejected requests. Use [conditions](/0-11-0/how-to/use-trigger-conditions) for body-based validation.
</Info>
