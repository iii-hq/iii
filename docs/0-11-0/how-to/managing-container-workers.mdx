...
You can manage the full lifecycle of container workers — add, configure, start, stop, inspect logs, and remove — using the `iii worker` CLI. Worker configuration is tracked in `config.yaml`; registry-managed worker source pins can also be tracked in `iii.lock`. See [Reproduce Worker Installs](/0-11-0/how-to/reproduce-worker-installs) for the lockfile workflow.
