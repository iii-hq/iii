  ...
  <AccordionGroup>
    <Accordion iconName="settings" title="Parameters">
      ...
      <ResponseField name="ops" type="UpdateOp[]" required>
        The list of atomic operations to apply. Each operation is a tagged object with a `type` field (`set`, `merge`, `increment`, `decrement`, `append`, or `remove`) and associated fields (`path`, `value`, `by`). For `set` / `increment` / `decrement` / `append` / `remove`, paths are first-level field names. For `merge`, `path` accepts either a single string (legacy / first-level field) or an array of literal segments for nested merge — see the [state worker docs](/0-11-0/workers/iii-state) for the full contract and validation rules. Use `path: ""` (or omit `path`) to target the root value.
      </ResponseField>
    </Accordion>
    <Accordion title="Returns">
      <ResponseField name="old_value" type="any">
...
`stream::update` uses the same update engine as [`state::update`](/0-11-0/workers/iii-state#error-codes). Each op may add an entry to the response `errors` array. Operations are best-effort: successfully applied ops still reflect in `new_value`, and failed ops are skipped.
