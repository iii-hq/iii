...
|--------|----------|------------|
| [HTTP](/docs/workers/iii-http) | HTTP trigger type, request/response handling | `rest_api` |
| [Queue](/docs/workers/iii-queue) | Async message processing with retries | `queue` |
| [Cron](/docs/workers/iii-cron) | Scheduled task execution | `cron` |
| [State](/docs/workers/iii-state) | Key-value state storage with atomic updates | `state` |
| [Stream](/docs/workers/iii-stream) | Real-time data streams with WebSocket push | `stream` |
| [PubSub](/docs/workers/iii-pubsub) | Publish/subscribe messaging | `pubsub` |
| [Observability](/docs/workers/iii-observability) | Structured logging, tracing, and metrics | `observability` |
| [Exec](/docs/workers/iii-exec) | Shell command execution | `exec` |
| [Bridge](/docs/workers/iii-bridge) | WebSocket bridge for SDK connections | `bridge` |
...
<Info title="Custom Workers">
  You can build your own Workers to integrate any service or infrastructure. See [Custom Workers](/docs/advanced/custom-modules) for a detailed guide.
</Info>
