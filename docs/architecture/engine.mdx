---
title: 'Engine'
description: 'The central orchestrator of a iii system — routes triggers to workers, loads configured engine workers, and tracks all registered functions.'
---
...
| **Worker orchestration** | Loads and initializes configured workers from `iii-config.yaml` |
...
| HTTP API | `3111` | HTTP endpoints registered by the HTTP worker (`iii-http`) |
