...
On Unix platforms, the Engine can re-read its config file at runtime without restarting the process. Send `SIGHUP` to the engine and it will diff the new config against the running worker set and apply only the workers whose entries actually changed.

```bash
kill -HUP $(pgrep iii)
```

The reload is atomic: if the new config is invalid, the running engine is left untouched.
...
| **Validate** | Dry-runs `create` + `initialize` on every added and changed worker without starting background tasks. Any failure rolls back the staged workers and aborts the reload |
| **Commit** | Stops changed workers (fire shutdown, destroy, remove registrations), drops removed workers, promotes validated replacements and additions |
...
```
reload: SIGHUP received, reloading from config.yaml
reload: diff +1 added, -0 removed, ~2 changed, =8 unchanged
reload: success
```

Failure paths log `reload: parse failed: ...`, `reload: validation failed for '<name>': ...`, or `reload: refused to remove mandatory worker '<name>'` and leave the running engine intact.

<Info title="Limitations">
  SIGHUP reload is Unix-only. Windows ignores SIGHUP. When the engine is started with `--use-default-config` there is no file to reload from, and SIGHUP logs `reload: ignored` and does nothing. In-flight invocations on changed or removed workers are not drained — they are dropped when the old worker is destroyed.
</Info>
