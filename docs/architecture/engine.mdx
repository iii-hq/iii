...
<Info title="Limitations">
  SIGHUP reload is Unix-only. Windows ignores SIGHUP. When the engine is started with `--use-default-config` there is no file to reload from, and SIGHUP logs `reload: ignored` and does nothing. In-flight invocations on changed or removed workers are not drained — they are dropped when the old worker is destroyed. A bad config causes the engine to exit — configure your process supervisor to NOT auto-restart on this exit code.
</Info>
