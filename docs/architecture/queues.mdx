...
| You need the function's return value right now | **No** | Use a [synchronous trigger](/docs/how-to/trigger-actions#1-synchronous-no-action) instead |
| The work is non-critical and losing it is acceptable | **Maybe** | [TriggerAction.Void()](/docs/how-to/trigger-actions#2-void-fire-and-forget) is simpler if you don't need retries |

<Info title="How-to guide">
  For queue modes, configuration, retries, and adapter guidance, see the [Queue worker reference](/docs/workers/iii-queue).
</Info>
...
<Info title="Full resource map">
  For the complete naming convention and examples, see the [Queue worker reference — Queue naming in RabbitMQ](/docs/workers/iii-queue#queue-naming-in-rabbitmq).
</Info>
...
<Info title="DLQ operations">
  For instructions on inspecting and redriving DLQ messages, see [Manage Failed Triggers](/docs/how-to/dead-letter-queues).
</Info>
...
You invoke the redrive function using the [`iii trigger`](/docs/how-to/trigger-functions-from-cli) CLI command against a running engine:
...
<Info title="Choosing an adapter">
  For a feature comparison of all queue adapters and scenario-based recommendations, see the [adapter comparison](/docs/workers/iii-queue#adapter-comparison).
</Info>

## Related

<CardGroup cols={3}>
  <Card title="Use Queues" href="/docs/how-to/use-named-queues" icon="list-check">
