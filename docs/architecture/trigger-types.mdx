...
sequenceDiagram
    ...
    participant BuiltinW as BuiltinWorker
    participant Engine
    ...
    Engine->>BuiltinW: Route trigger to worker
    BuiltinW-->>Engine: Trigger ready

    External->>BuiltinW: Event arrives (HTTP request / cron tick / queue message)
    BuiltinW->>Engine: Fire trigger
...
sequenceDiagram
    ...
    participant BW as BuiltinWorker
...
The engine sets up the trigger in the appropriate built-in worker. When an HTTP POST request comes to `/users`, the `users.create` function will be invoked.

Trigger types are registered by built-in workers during engine initialization:
...
sequenceDiagram
    ...
    E->>BW: Initialize Worker
    BW->>R: RegisterTriggerType("http")
    BW->>R: RegisterTriggerType("queue")
    BW->>R: RegisterTriggerType("cron")
...
Workers can register custom trigger types by implementing the trigger interface:
...
<Card title="Workers" href="/modules" icon="cubes">
  Explore workers that provide trigger types
</Card>
