...
Install a file watcher (e.g., `watchdog`) and configure the worker command. In your `config.yaml`, configure the worker under `iii-exec`:
...
`watchmedo` (ships with `watchdog`) restarts the process on any `.py` change under `src/`.
...
Update your production config to run the installed module:
...
- [ ] Install `watchdog` for development file watching
...
- [ ] Replace `motia dev` with `watchmedo auto-restart ... -- python -m src.main` in your dev config
