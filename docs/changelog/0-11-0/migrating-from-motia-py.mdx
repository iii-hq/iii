...
- **Connect from the browser** using `iii-browser-sdk` with [Worker RBAC](/how-to/worker-rbac) for secure, real-time frontends — no REST layer needed. See [Use the Browser SDK](/how-to/use-iii-in-the-browser).
