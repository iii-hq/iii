...
<Update label="0.22.0" description="July 2026">
  ...
  `RegisterTrigger` no longer fails when its trigger type isn't registered yet. The engine parks the registration intent in `pending_triggers`, logs `[PENDING]`, and acks success; when the trigger type registers — or a provider re-registers after a restart — matching intents drain and activate. A provider disconnect moves its live bindings back to pending (`[DISABLED]`) and they recover on re-register. Consumers that connect before providers, and provider restarts that used to drop other workers' bindings, are now both safe. A bind that reaches the engine while the type *is* registered but whose activation fails because the provider connection is closing is now parked and replayed on the next type registration too, instead of being silently dropped until the binding worker restarts.

  ## `TriggerAction.Enqueue` routes through the standalone queue worker

  Enqueue actions are delivered to the registered `engine::queue::enqueue` provider instead of the in-process queue module. Engine-owned receipt IDs are preserved and `traceparent` / `baggage` propagate to queued work; a missing or failing provider fails closed with `enqueue_error` — there is no silent in-process fallback. `durable:subscriber` and `TriggerAction.Enqueue` now require the standalone `queue` worker. The legacy in-engine `iii-queue` builtin was retired from the default config but remains available **opt-in**: an explicit `iii-queue` entry in `config.yaml` still boots it (`enabled_by_default = false`).

  ## Per-subscriber queue retry tuning is honored
  ...
  `iii.worker.yaml` accepts optional `tags` publish metadata — a list of strings surfaced in the generated schema for registry discovery — with clear validation errors on malformed entries. It does not change worker runtime behavior.
  ...
  - Managed builtin workers drop the `iii-` prefix (except `iii-directory`); references use the shorter `http`, `state`, `queue`, `cron`, and `pubsub` names.
</Update>
