...
<Update label="0.19.5" description="June 2026">
  ...
  As with the other workers, the config.yaml block is **seed-only** once a value is persisted; change settings via `configuration::set` or by editing the persisted file (`./data/configuration/iii-pubsub.yaml` with the default `fs` adapter). `${VAR:default}` placeholders expand on read. The schema carries a per-field description and rejects unknown top-level keys at `configuration::set` time.
</Update>
