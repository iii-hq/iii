...
<Update label="0.24.3" description="September 2026">
  ## Configuration overrides through the configuration service

  - Compose no longer writes an execution snapshot file or sets `III_CONFIG`. Before a worker
    starts, Compose reads the current entry with `configuration::get` (`raw: true`), merges package
    defaults and `config_override`, and delivers the result with `configuration::set` and
    `flush: false`. Workers read the merged value through `configuration::get` using the id in
    `III_CONFIG_NAME`.
  - `configuration::set` accepts `flush`, which defaults to `true`. `flush: true` validates and
    persists the complete submitted object. `flush: false` updates only the active in-memory value,
    schedules no later disk write, and may precede worker registration; validation uses the schema
    when available, and reads validate again after registration. Both forms notify consumers.
  - `configuration::ensure` and metadata registration seed defaults into the persisted base only.
    They preserve an active in-memory value and do not deliver base defaults in its place.
    `configuration::migrate` moves an active in-memory value to the new id without saving it.
  - Removing a field from `config_override` keeps its current active value instead of restoring
    the value on disk. Stopping a worker leaves the active value in place, and restarting the
    configuration service discards unsaved values and reloads its adapter. An explicit save with
    `flush: true` persists every submitted value, including values that came from an override.
    Distinct concurrent overrides that share one configuration id are not supported.
  - `III_CONFIG` is retired. Declaring it in `environment` or an `env_file` fails with
    `RETIRED_CONFIG_ENV`, and stale inherited values are removed from the worker environment.
    The daemon now owns seven reserved variables instead of eight.

  <Callout type="warning">
    Workers that read the file named by `III_CONFIG` must read their configuration with
    `configuration::get` and `III_CONFIG_NAME` instead. Remove `III_CONFIG` from `environment` and
    `env_file` declarations before upgrading, or startup fails with `RETIRED_CONFIG_ENV`.
  </Callout>

  See [Worker Compose](/using-iii/compose) and [Configuration](/using-iii/configuration) for the
  delivery order and the `flush` semantics.

  ## Managed engine lifetime

  - The engine that `iii compose --up` starts now exits with the Compose process that owns it,
    including when Compose is killed with `SIGKILL` or dies before the engine is ready, so a later
    start can bind the same port cleanly. The engine watches a private stdin lifeline that workers
    do not inherit. Workers still survive a daemon crash and are re-adopted on restart.
  - `SIGHUP`, sent when a terminal window closes or an SSH session drops, now triggers the same
    graceful shutdown as `SIGTERM`. Previously it ended the daemon before teardown and left the
    managed engine serving on its port with no owner.

  ## Engine channel sockets

  - When a channel's reader leaves, the engine drops the channel and closes the writer socket
    immediately, even if the writer never attached. For example, the http worker drops its response
    reader when a function returns a buffered value. These sockets were previously held until the
    TTL sweeper removed them minutes later.
  - Channels created with `engine::channels::create` belong to the calling connection, whose
    disconnect releases the ends nobody has attached. In-process callers still rely on the sweeper.
  - On Unix, the engine raises its soft open-file limit toward the hard limit at startup (capped at
    10240 on macOS). Terminals on macOS start processes with a soft limit of 256, which a burst of
    HTTP traffic could exhaust, making `accept` fail with `EMFILE`.

  ## Product telemetry

  - Amplitude is removed; product-usage events go only to PostHog. `amplitude_api_key` is removed
    from `TelemetryOptions` in the Node, browser, Python, and Rust SDKs.
  - Compose reports `compose_up_finished` for each foreground `iii compose --up`, up to three
    `compose_container_failed` events for containers that did not become ready, and
    `compose_op_finished` for later up, down, add, remove, restart, and update operations. Events
    carry outcomes, durations, counts, and fixed error codes; they contain no paths, container
    names, worker references, or error messages. A failing command waits at most two seconds for
    its report.
  - The engine reports harness session usage that the `harness` worker publishes on the durable
    `harness:usage` topic: `harness_session_progress` at root turns 1, 2, 5, 10, 25, and 50, and
    `harness_turn_failed` once per session and outcome. It subscribes in every namespace that
    provides `durable:subscriber`.
  - An email address entered in the onboarding signup box or the Console prompt, published on
    `email:signup`, is validated, lowercased, and attached to this device's PostHog person through
    a `user_identified` event. Identities are not aliased or merged.
  - When telemetry is disabled, nothing is sent: Compose events are skipped, and the engine still
    consumes and discards `harness:usage` and `email:signup` messages so the durable queue does not
    accumulate a backlog it would send later.

  ## Docs and agent skills

  - The website, the `AGENTS.md` sources, and the bundled `skills/` use `compose::add` to add
    workers and `iii project init` to create projects, and the website includes a new AI
    onboarding prompt.

  ## Migration

  - Rust: remove `amplitude_api_key` from `TelemetryOptions` struct literals.
  - Python: stop passing `amplitude_api_key` to `TelemetryOptions`.
  - Node: remove `amplitude_api_key` from `telemetry` options; TypeScript reports it as an unknown
    property.
  - Workers: replace reads of `III_CONFIG` with `configuration::get` on `III_CONFIG_NAME`.

  [Compare 0.24.2 to 0.24.3](https://github.com/iii-hq/iii/compare/iii%2Fv0.24.2...iii%2Fv0.24.3).
</Update>

<Update label="0.24.2" description="September 2026">
  ## Readable configuration names

  - Without an explicit `config_name`, the generated configuration id is exactly
    `<namespace>-<container-key>`. For example, `default` plus `harness` uses `default-harness`,
    stored by the filesystem adapter as `config/default-harness.yaml`, instead of the hashed
    `default-harness-a14f3656efb8d5ea` form introduced in 0.24.1. Explicit names are unchanged.
  - Generated ids must match `[a-z0-9_-]{1,64}`. Compose rejects an invalid or long generated name
    with `INVALID_CONFIG_NAME` and never sanitizes, truncates, or hashes it; set an explicit
    `config_name` instead.
  - Within one project, a generated name that collides with another container's explicit name is
    rejected before startup with `CONFIG_NAME_COLLISION`. Sharing requires both names to be
    explicit. Across namespaces, `a-b` / `c` and `a` / `b-c` both produce `a-b-c`, so choose
    unambiguous namespaces or distinct explicit names.
  - `III_CONFIG_NAME` is always present and carries the explicit id or
    `<namespace>-<container-key>`.

  ## Configuration migration

  - Before reading configuration or starting a worker, Compose migrates the exact hashed id the
    previous algorithm produced for that namespace and key. The new `configuration::migrate`
    (`from_id`, `to_id`) preserves raw values (including `${VAR}`, `false`, `0`, and `null`), name,
    description, metadata, and schema, and returns `{ action, entry }` with `migrated`,
    `preserved`, or `missing`. A successful move emits source `:deleted` and destination
    `:registered`; no-ops emit nothing.
  - The legacy source takes priority over an existing destination. After that, the `default`
    namespace adopts an entry named with the bare container key (`state` becomes `default-state`),
    unless another container explicitly owns it. Other namespaces and explicit `config_name`
    values never adopt bare entries.
  - The filesystem adapter archives the original source as `<source>.yaml.bak`; the previous
    destination is replaced, not backed up. Files ending in `.yaml.bak`, `.bak.yaml`, or
    `.bkup.yaml` are ignored during loading, watching, and legacy directory migration. A
    conflicting backup is never overwritten and stops migration with an error.
  - Over the bridge adapter, migration is delegated to the remote authority. Compose and every
    bridge hop check `configuration::migration-capabilities` first; an authority without
    `configuration::migrate` fails with `CONFIG_MIGRATION_FAILED` rather than falling back to a
    read, copy, and delete that could reset stored values.

  <Callout type="warning">
    Upgrade the configuration authority, including a remote engine behind the bridge adapter,
    together with Compose. Stop consumers of the source entry before migration, and do not run two
    configuration authorities against the same directory. The filesystem adapter requires
    same-directory hard-link support and may normalize YAML formatting or remove comments during
    the one migration rewrite; values and unknown document fields are retained.
  </Callout>

  ## Runtime-only overrides

  - `config_override` became execution-only: Compose wrote the merged value to the private
    `III_CONFIG` snapshot and never persisted it to `config/<id>.yaml` or the configuration
    service. Removing an override restored the stored setting on the next start. 0.24.3 replaces
    the snapshot file with in-memory delivery through the configuration service.
  - Configuration read failures stop startup with `CONFIG_FETCH_FAILED` instead of silently
    falling back to defaults.

  ## Guided tour

  - An image prefetch that finishes while the API key prompt of `iii project init --learn-iii` is
    on screen no longer cancels the prompt as if the operator had pressed `Ctrl+C`.
  - The tour's onboarding pane opens again. Project setup adds `config_name: console` to the `ade`
    container in the scaffolded `worker-compose.yaml`, so the Console reads the layout entry the
    tour seeds instead of the derived `default-ade`. A container that already sets
    `config_name` is left alone.

  See [Worker Compose](/using-iii/compose) for configuration names and migration.

  [Compare 0.24.1 to 0.24.2](https://github.com/iii-hq/iii/compare/iii%2Fv0.24.1...iii%2Fv0.24.2).
</Update>

<Update label="0.24.1" description="September 2026">
  ## Security

  - Node OpenTelemetry packages move to 2.11 / 0.222 in `@iii-dev/helpers` and `iii-sdk`.
    Rust moves to `opentelemetry` 0.32, `opentelemetry_sdk` 0.32.1, and
    `tracing-opentelemetry` 0.33 in `iii-helpers`, `iii-sdk`, and the engine. These updates close
    GHSA-8988-4f7v-96qf and GHSA-w9wp-h8wv-79jx, which allow unbounded allocation while parsing
    W3C `baggage` headers. 0.24.0 still shipped the previous versions.
  - Node SDKs update to `ws` 8.21 to address GHSA-96hv-2xvq-fx4p and GHSA-58qx-3vcg-4xpx.
  - The engine updates `hickory-resolver` and `hickory-proto` to 0.26 to address
    GHSA-3v94-mw7p-v465 and GHSA-q2qq-hmj6-3wpp, and refreshes transitive crates including
    `quinn-proto`, `h2`, `rustls`, `tar`, and `serde_with`.
  - CI uses the committed Cargo lockfile, tests `iii-helpers` and `@iii-dev/helpers` directly,
    and gates dependency advisories with `cargo-deny` and `pnpm audit` over the SDK packages.
    Dependabot adds weekly grouped updates for Cargo, npm, and GitHub Actions.

  ## Compose environment and configuration isolation

  - Workers and lifecycle hooks inherit the machine environment visible to the Compose daemon,
    even without `env_file` or `environment`. Precedence is machine environment, then env files
    in declaration order, then the worker's environment map, then daemon-owned reserved values.
    Empty environment-map values still preserve values loaded from env files.
  - `III_HOST_USER_ID` comes from the current project's `.iii/project.ini`, not the machine
    environment. An explicit `env_file` or `environment` value can override it.
  - Without an explicit `config_name`, Compose assigns a stable configuration ID derived from
    the effective namespace and container key. Workers receive it through `III_CONFIG_NAME`,
    even before their first configuration value exists, so projects on a shared engine no longer
    collide on a worker's default configuration ID.
  - Compose merges package defaults, the stored configuration, and `config_override` in that
    order, then publishes the result to both the configuration service and the `III_CONFIG`
    file before starting the worker. A missing entry permits defaults; a configuration-service
    failure stops startup instead of silently using stale defaults.

  <Callout type="warning">
    Inherited machine variables can include credentials. Only start workers you trust with the
    daemon's environment, and export variables before starting the daemon; changes in another
    shell do not update an already-running daemon. Workers that use the configuration service
    must honor `III_CONFIG_NAME`. Set `config_name` explicitly when an existing configuration ID
    must be retained or intentionally shared.
  </Callout>

  See [Worker Compose](/using-iii/compose) for environment precedence and configuration delivery.

  ## Atomic configuration defaults

  - `configuration::ensure` creates or refreshes an entry and seeds `initial_value` only when no
    non-null value is stored. Existing values, including `false`, `0`, and `""`, are preserved.
    Use it instead of read-then-`configuration::register` when workers initialize defaults.
    The response includes the stored `entry` and an `action`: `seeded`, `preserved`, or `registered`.
  - Configuration mutations are serialized within one engine's local store, so concurrent seeds
    and writes do not overwrite a value based on a stale read. Filesystem watcher events reread
    authoritative storage before updating the cache, preventing stale events from reverting a
    newer local write or deleting a recreated entry.
  - With the bridge adapter, `configuration::ensure` forwards the original seed to the remote
    engine, which decides whether to preserve or seed the value. An older remote without this
    function returns `ADAPTER_ERROR`; there is no fallback that could overwrite remote settings.

  ## Installation and release tooling

  - The installer accepts `--non-interactive` or any nonempty `III_NON_INTERACTIVE` value to
    suppress setup prompts even when a terminal is attached. It installs the binaries and prints
    the setup command instead of running onboarding. Runs without a terminal already behave this
    way. See [Installer options](/install#installer-options).
  - The release workflow accepts a validated `version_override`, allowing a stable patch release
    after a higher prerelease version has been selected. Overrides must be newer than the latest
    stable tag and must not reuse an existing tag. They publish the current `main` code, not a
    historical revision or a backport.

  ## Migration

  - Rust: if your crate depends on `opentelemetry` directly, move to 0.32 and
    `tracing-opentelemetry` 0.33. Prefer `iii_helpers::observability::opentelemetry` so the version
    matches the SDK.
  - Node: no SDK API change. Remove pnpm patches or overrides that forced `@opentelemetry/*`
    2.x on older `@iii-dev/helpers` releases when upgrading to 0.24.1.

  [Compare 0.24.0 to 0.24.1](https://github.com/iii-hq/iii/compare/iii%2Fv0.24.0...iii%2Fv0.24.1).
</Update>

<Update label="0.24.0" description="September 2026">
  ## Compose package locks and configuration

  - Compose writes `worker-compose.lock` with the resolved dependency graph, exact versions,
    artifact URLs, SHA-256 digests, and package defaults. Starts and restarts reuse the lock rather
    than resolving moving tags again, and cached package contents are verified before reuse.
  - `iii compose build --frozen`, `iii compose --up --frozen`, and `compose::up frozen=true` require
    the compose file and lock to match without resolving selectors or changing the lock. Commit
    the lock so development, CI, and deployments use the same package content.
  - `compose::add` accepts container objects alongside worker names, including scripts,
    dependencies, environment files, configuration overrides, working directories, and timeouts.
    Changed declarations restart in place; unchanged workers stay running.
  - `compose::update` without a worker selection updates all declared package workers to the
    registry's latest version and skips local workers. A targeted update resolves the selector
    already declared; use `worker=name@latest` to move an exact pin to the latest channel.
    Failed resolution or download leaves the previous lock and running workers unchanged.
  - Dependency resolution preserves local workers and declared pins, warns on aliases, and reuses
    canonical dependencies instead of adding duplicate workers.

  ## Compose lifecycle and upgrade notes

  <Callout type="warning">
    Workers are now optional by default. Set `required_default: true` at the top of
    `worker-compose.yaml`, or `required: true` on individual containers, to fail startup and roll
    back when a required worker cannot start. Optional failures appear in `not_required_failures`
    even when the operation succeeds; `start_after` controls startup order, not whether a worker
    requires its dependency to succeed.
  </Callout>

  - Restart policies support `no` (the default), `on-failure`, and `always`, with configurable
    backoff, attempt limits, and a stability window. Eligible retries keep dependents running;
    status reports `restarting` between attempts.
  - `compose::add`, `compose::update`, and `compose::remove` accept caller-selected `operation_id`
    values and return an acceptance response before completion. Bind `compose-operation` with
    `terminal_only: true` before submitting the mutation, then read `compose::operation` once for
    recovery. Acceptance is not readiness, and completion messages now report optional worker
    failures rather than claiming every worker is ready.
  - Runtime state and logs move to `<project-dir>/.iii/compose/<namespace>/`; the package cache
    remains shared. `III_COMPOSE_STATE_DIR` can redirect state to a writable location.
  - Startup shows engine, worker, and download progress, plus restart timing. Package metadata
    resolves concurrently, duplicate progress rows are removed, and `Ctrl+C` cancels startup.
    Linux process labels include the namespace to distinguish Compose daemons and their engines.
  - Blank `environment` entries no longer erase values loaded from `env_file`. Switching a
    `config_override` mapping's `name` replaces the whole variant instead of retaining settings
    from the previous adapter. Worker edits also handle YAML comments correctly.
  - Compose retries transient `ETXTBSY` spawn failures, avoids spurious `Gone` errors during
    restarts, and rejects a container argument to `compose::stop` rather than stopping the daemon
    when a caller intended to stop one worker.

  <Callout type="warning">
    Before upgrading, stop existing Compose daemons with the old version and confirm their engines
    and workers have stopped. Old process records, logs, and VM data are not migrated; preserve
    anything you need. Add `**/.iii/compose/` to `.gitignore` to exclude generated project state.
    See [Worker Compose](/using-iii/compose) for the state layout and lifecycle settings.
  </Callout>

  ## Engine, CLI, and SDK reliability

  - Function-targeted triggers registered through `engine::register_trigger` now belong to the
    caller's namespace. Provider lookup and `engine::triggers::info` try that namespace before
    falling back to `default`, instead of leaving valid registrations pending indefinitely.
  - `engine::registered-triggers::list` accepts `include_pending: true`, and trigger inspection
    reports `active` or `pending` status to make missing providers easier to diagnose.
  ...
  - `iii trigger` gains `--engine` and honors `III_URL` and the working directory's compose file;
    `--address` and `--port` are deprecated. Compose also honors `III_URL` as an external engine
    selection and does not start a second engine when it is set.
  ...
  - Read-only trace and log functions remain discoverable through `engine::functions::list`
    without `include_internal`; destructive clear operations remain hidden by default.

  ## Installation and worker compatibility

  - `iii project init --learn-iii` creates a harness project and starts the guided tour. The
    installer offers the tour after installation and accepts a single keypress at its prompt.
  - The installer adds `--start-with` to start a harness project with selected workers,
    `--need-envs` to request their environment variables, and `--skip-bin-download` to run setup
    against an existing installation. `--need-envs` requires `--start-with`. Each selected worker
    receives the full project `.env` file, so use these options only with workers you trust.
    See [Installer options](/install#installer-options).
  - The bundled `iii-worker` supports Alpine/musl with host-aware GNU or musl artifact selection
    during install and update. VM execution still requires KVM and an appropriate guest rootfs.
  - Node and Python worker images separate lean runtime images from builder images, and worker
    manifests accept the registry's `license` key when `runtime.base_image` is set.

  ## Telemetry and tutorials

  - `III_TELEMETRY_ENABLED` accepts `false`, `0`, `no`, and `off` regardless of case or surrounding
    whitespace. This controls product-usage telemetry, not OpenTelemetry observability.
  - Product telemetry reports completed onboarding tour steps and deduplicates repeated step
    messages within an engine run.
  - The [Linkly tutorial](/tutorials/linkly/overview) separates Agentic and Exploration paths, and
    installation docs use the harness template and Compose to add inference providers.

  [Compare 0.23.0 to 0.24.0](https://github.com/iii-hq/iii/compare/iii%2Fv0.23.0...iii%2Fv0.24.0).
</Update>
...
<Update label="0.22.1" description="August 2026">
  ...
  The queue worker now rebuilds the adapter selected in its persisted configuration before starting consumers. After the first boot removes the one-time `config.yaml` seed, a queue configured for a non-default transport such as RabbitMQ no longer restarts with the default transport while retaining the stored queue settings.

  ## `iii-*` helper binaries resolve consistently from `PATH`

  Every engine path that launches a sibling binary, including `iii-worker` and `iii-console`, now uses the same `PATH`-first resolver. A development or override binary earlier on `PATH` takes precedence, with the managed installation used as a fallback. `iii update` continues to manage the installed copy directly.

  ## Worker installs support large dependency graphs

  `iii worker add` and `iii worker reinstall` no longer reject valid dependency graphs because they are deep or contain more than 32 workers. Graphs are validated for duplicate or unreachable workers, missing dependency targets, and cycles without an arbitrary depth cap. Installs above 32 workers require explicit confirmation; pass `--yes` in CI or other non-interactive environments.

  ## Sandboxes keep active workloads responsive

  Sandboxes now admit a bounded number of concurrent `sandbox::exec` calls (four by default) instead of serializing every command. Active execs and relayed TCP, UDP, or DNS payload keep a sandbox from being reaped as idle, while a hard per-exec timeout still bounds cleanup. TCP relays also avoid delayed packet coalescing that could add about 40 ms to overlapping request-response traffic. Concurrent execs share the guest filesystem and process table, so callers that mutate shared state must coordinate access.

  ## Observability triggers start reliably and preserve metadata

  Trace trigger subscribers now wait for span storage to become available instead of silently stopping when engine startup wins the initialization race. Trace and log trigger deliveries also preserve the metadata registered with the trigger, so metadata-routed consumers receive the context they expect.

  ## Additional reliability fixes

  - Empty `traceparent`, `tracestate`, and `baggage` values are treated as absent, preventing invalid W3C baggage warnings in the engine and Rust observability helpers.
  - RabbitMQ DLQ inspection and redrive operations use isolated channels, so inspecting a missing queue cannot close the channel used for publishing or consuming messages.
</Update>

<Update label="0.22.0" description="July 2026">
  ## Recoverable triggers survive startup races and provider restarts

  `RegisterTrigger` no longer fails when its trigger type isn't registered yet. Consumers that connect before providers, and provider restarts that used to drop other workers' bindings, are now both safe.

  The engine optimistically stores the registration intent in `pending_triggers`, logs `[PENDING]`, and acks success; when the trigger type registers — or a provider re-registers after a restart — the triggers are registered appropriately to the worker.

  A provider disconnect moves its live bindings back to pending (`[DISABLED]`) and they recover on re-register. A bind that reaches the engine while the type *is* registered but whose activation fails because the provider connection is closing is now stored optimistically and replayed on the next type registration too.

  ## `TriggerAction.Enqueue` routes through the standalone queue worker

  Enqueue actions are delivered to the registered `engine::queue::enqueue` provider instead of the in-process queue worker. Engine-owned receipt IDs are preserved and `traceparent` / `baggage` propagate to queued work; a missing or failing provider fails closed with `enqueue_error` — there is no silent in-process fallback. `durable:subscriber` and `TriggerAction.Enqueue` now require the standalone `queue` worker. The legacy in-engine `iii-queue` builtin was retired from the default config but remains available **opt-in**: an explicit `iii-queue` entry in `config.yaml` still boots it (`enabled_by_default = false`).

  ## Per-subscriber queue retry tuning is corrected

  The builtin queue adapter now applies `queue_config.maxRetries` and `queue_config.backoffDelayMs` per `durable:subscriber` trigger; previously every failing delivery got a fixed 3 attempts with default backoff regardless of config. Tuning is overlaid at retry-decision time, so it also covers jobs enqueued before the subscriber connected. `maxRetries: N` means N retries after the initial attempt (N+1 attempts total); `maxRetries: 0` fails to the DLQ after one attempt, matching the RabbitMQ adapter's counting.

  ## `engine::observability` is now a required engine worker

  Observability is always injected — like `configuration` — with defaults living in `ObservabilityWorkerConfig::default()`. `iii worker add` no longer writes an observability `config:` block into `config.yaml`, and first boot always gets a valid configuration seeded to `./config/iii-observability.yaml`. `${VAR:default}` placeholders (including `__III_ENGINE_VERSION__`) are expanded when publishing the live OTEL config.

  ## `engine::functions::list` filters by multiple workers

  The call gains a `workers: [...]` filter alongside the existing single `worker: String` field, listing functions owned by any of several workers in one request. When both are set they OR-combine.

  ## Registry tags in worker manifests

  `iii.worker.yaml` now accepts optional `tags` metadata: a list of strings surfaced in the generated schema for registry discovery. It does not change worker runtime behavior.

  ## Reconnect and worker-lifecycle hardening

  A batch of fixes closes reconnect and first-boot failure modes:

  - Worker reconnects no longer trip an ownership kill loop that could tear down a freshly reconnected worker.
  - First-boot worker zombies from VM transport wake starvation and SDK reconnect death are fixed; external workers are supervised and the Rust SDK reconnect path is hardened.
  - Accepted worker WebSocket sockets that never complete the upgrade are no longer held open by the idle timeout.
  - The Node SDK bounds WebSocket connect attempts and detects half-open sockets.
  - Spawned builtin daemons receive the engine's actual WebSocket address.
  - Registry binary workers now inherit the engine's `III_ENGINE_URL` / `III_URL` when the `iii-worker-manager` runs on a non-default port, so they connect instead of assuming the default port; `iii-worker start`/`restart` resolve the manager port from `config.yaml` when `--port` is omitted.

  ## Managed worker names propagate to the SDKs via `III_WORKER_NAME`

  `iii-worker` injects the `config.yaml` entry name as `III_WORKER_NAME` into every managed start path (libkrun, OCI, binary), and the Node, Python, and Rust SDKs default their reported worker name from it. A worker whose code sets no name now matches its config entry instead of self-reporting `hostname:pid` and timing out on `iii worker add --wait`. An explicit `workerName` / `worker_name` still wins; the manifest `env:` block cannot override the injected value.

  ## `iii worker` CLI audit

  A full pass over the CLI surface (every operation except `add`) fixed: `remove` now deletes the `iii.lock` entry so removed workers stay removed; `status` terminates on `EngineDown` instead of hanging forever and returns non-zero exit codes so it works as a scriptable readiness probe; `clear` (all) also wipes `~/.iii/managed`; `update` takes the same project lock as `sync` and prints a friendly message when `iii.lock` is missing; `logs` drops the dead `--address`/`--port` flags and returns consistent exit codes; `sandbox create`/`run` fail fast when the engine is down instead of pulling a full image first. `iii worker list` / `status` also reflect engine truth rather than local config. A hostless `iii worker add` run from a non-project directory (no config file, no exported `III_CONFIG_PATH`) now installs through the running local engine instead.

  ## Python SDK: connection-state listener

  `III.add_connection_state_listener` subscribes to connection-state changes, and the SDK warns when the initial connect wait times out.

  ## Security: SSRF via IPv4-mapped IPv6 addresses

  `is_private_ip` in the URL validator classified IPv4 and IPv6 separately, so an IPv4-mapped IPv6 address like `::ffff:169.254.169.254` bypassed the private-range checks and could reach loopback, RFC1918 ranges, or the cloud metadata endpoint. Mapped addresses are now unwrapped to their underlying IPv4 and classified with the same rules.

  ## Breaking: `RegisterFunction::new_async_with_bad_request` removed (Rust SDK)

  The `new_async_with_bad_request` constructor is removed from the Rust SDK — source-breaking for external consumers that constructed it directly. Wire behavior is unchanged: malformed `worker::*` payloads still return the W105 `BadRequest` envelope with a `worker::schema` hint, a policy that now lives entirely in the worker daemon rather than the SDK.

  ## Additional fixes

  - Node and browser SDKs default trigger generics so untyped `trigger` calls compile under `tsc`.
  - The browser SDK exports `IIIReconnectionConfig` and `RegisterFunctionFormat`, and generates invocation UUIDs in insecure (non-HTTPS) contexts.
  - The engine rejects duplicate migrated builtin workers and warns on deprecated builtin names.
  - The Docker image boots cleanly: `/app/config` is pre-created for the non-root user, and the config seed-strip no longer logs spurious warnings on a read-only mount.
  - `iii worker add` no longer rejects a valid bundle whose transitive dependencies share nodes — the dependency-graph bounds check counts unique nodes instead of distinct paths, so a wide shared graph stops overflowing the `edge_traversal` guard (`limit 64, found 65`).
  - Some workers drop the `iii-` prefix: `http`, `state`, `queue`, `cron`, and `pubsub` names.
</Update>
