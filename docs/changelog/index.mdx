...
<Update label="0.24.3" description="September 2026">
  ## Configuration overrides reach workers through the configuration service

  - Workers receive `config_override` values the same way they read any other configuration:
    through `configuration::get` on the id in `III_CONFIG_NAME`. Compose merges package defaults,
    the stored entry, and `config_override`, and hands the result to the configuration service
    before the worker starts. There is no longer a separate snapshot file to read.
  - Override values live in memory only and do not change `config/<id>.yaml`. Saving the
    configuration, for example from the Console or with `configuration::set`, persists the whole
    submitted value, including fields that came from the override.
  - Removing a field from `config_override` does not bring back the value on disk. The worker keeps
    the current value, across its own restarts, until it is changed or the configuration service
    restarts, which discards unsaved values and reloads them from disk.
  - `configuration::set` accepts `flush: false` to update only the active value without writing to
    disk. Defaults seeded with `configuration::ensure` never replace an active override. Two
    containers that share one `config_name` must not declare different overrides.
  - `III_CONFIG` is retired. Declaring it in `environment` or an `env_file` fails startup with
    `RETIRED_CONFIG_ENV`.

  <Callout type="warning">
    If a worker reads the file named by `III_CONFIG`, switch it to `configuration::get` with
    `III_CONFIG_NAME` before upgrading, and remove `III_CONFIG` from `environment` and `env_file`
    declarations.
  </Callout>

  See [Worker Compose](/using-iii/compose) and [Configuration](/using-iii/configuration) for
  details.
  ...
  - The engine that `iii compose --up` starts now exits with the Compose process that owns it,
    ...
    do not inherit. Workers still survive a crash of the Compose daemon (`iii compose`) and are
    re-adopted when it restarts.
  ...
  - Under bursts of HTTP traffic on macOS, the engine no longer stops accepting connections with
    `EMFILE`. Terminals on macOS start processes with a soft limit of 256 open files, and each HTTP
    request holds two channels. On Unix, the engine now raises its soft open-file limit toward the
    hard limit at startup (capped at 10240 on macOS).
...
<Update label="0.24.2" description="September 2026">
  ...
  - On the first start after upgrading, Compose renames each worker's stored configuration to the
    readable id and keeps its values, including `${VAR}` placeholders. The hashed id from 0.24.1
    (`default-harness-a14f3656efb8d5ea`) becomes `default-harness`. In the `default` namespace, an
    entry named after the bare container key, as earlier releases stored it (`state`), becomes
    `default-state`. Workers with an explicit `config_name` are not migrated.
  - When both the old and the new entry exist, the old entry wins. The filesystem adapter keeps the
    original file as `<old-id>.yaml.bak`, and backup files are ignored when configuration loads.
    The one migration rewrite may reformat the YAML and drop comments.
  - Migration runs in the configuration service through the new `configuration::migrate`. With the
    bridge adapter, upgrade the remote engine together with Compose; an older remote fails startup
    with `CONFIG_MIGRATION_FAILED` instead of resetting stored values to defaults.
