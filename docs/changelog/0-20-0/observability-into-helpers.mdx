...
The shim covers the public API only. Symbols that were already marked internal (such as `getMeter` / `getTracer` in Node, moved to `@iii-dev/observability/internal` in 0.17.0) are not re-exported from the shim.
