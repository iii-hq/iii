...
The `cron` trigger type fires a registered function on a schedule. The handler receives `{trigger, job_id, scheduled_time, actual_time}` as its payload. Work is typically fanned out by calling `durable::publish` to downstream queue handlers.

```mermaid
graph LR
    Cron -->|"every minute"| CronFn
    CronFn -->|"durable::publish: job.tick"| JobHandler
...
iii.register_function((RegisterFunctionMessage::with_id("cron.periodic_job".into()), |_input| async move {
    ...
    iii.trigger(TriggerRequest::new("durable::publish", json!({
        "topic": "job.tick",
        "data": {
...
- Keep cron handlers lightweight. Use `durable::publish` to fan out work to queue handlers that can run in parallel.
