...
- For partial mutations, prefer `state::update` with atomic ops (`set`, `merge`, `append`, `increment`, `decrement`, `remove`) over read-modify-write with `state::set`. The `merge` op accepts either a single string (first-level field) or an array of literal segments for nested merge. See [iii-state worker reference](/docs/workers/iii-state#functions) for the ops table and [Build per-session structured state with nested merge](/docs/how-to/manage-state#build-per-session-structured-state-with-nested-merge) for a full example.
