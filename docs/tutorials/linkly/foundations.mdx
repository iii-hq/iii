...
short-code → URL mappings in a key-value store (provided by `state`). Add both now so they're ready
when the `link` worker reaches for them:
...
The generated root `worker-compose.yaml` declares how `link` builds and runs, including exact
toolchain versions, bundle files, a digest-pinned base image, and runtime argv. The generated
descriptor works for this tutorial.

<Info>
  iii runs an isolated sandboxed worker from the compiled descriptor, but a worker can also be an
...
On this first start the engine also creates a `config/` directory with one file per worker
...
```bash
# Declare this local worker under workers: and reference catalog://<slug> in the stack.
```
