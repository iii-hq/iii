...
an `analytics/` worker with a `src/main.py` example and adds it to the root Compose catalog.
...
The generated `workers.analytics` descriptor in the root `worker-compose.yaml` works for our
purposes. Validate it with `iii compose validate --file worker-compose.yaml` before starting the
stack.
