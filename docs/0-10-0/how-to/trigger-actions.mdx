...
`TriggerAction.Enqueue({ queue: 'name' })` routes the invocation through a **named queue** configured in `iii-config.yaml`, check how to [create queues in more detail](/0-10-0/how-to/use-queues). The caller receives an acknowledgement (`messageReceiptId`) once the engine accepts the job but does not wait for it to be processed. The queue provides retries, concurrency control, backoff, and optional FIFO ordering. If all retries are exhausted, the job moves to a dead letter queue.
...
<CardGroup cols={2}>
  <Card title="Use Queues" href="/0-10-0/how-to/use-queues" icon="bolt">
  ...
  <Card title="Functions & Triggers" href="/0-10-0/how-to/use-functions-and-triggers" icon="code">
    Register functions and bind triggers to them
  </Card>
  <Card title="Trigger Types" href="/0-10-0/how-to/use-functions-and-triggers#trigger-types" icon="diagram-project">
