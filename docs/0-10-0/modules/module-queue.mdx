...
<Info title="How-to guidance">
  For step-by-step instructions on configuring and using queues, see [Use Queues](/0-10-0/how-to/use-queues). For DLQ management, see [Manage Failed Triggers](/0-10-0/how-to/dead-letter-queues).
</Info>
...
The producer targets the function and queue explicitly. Queue configuration is centralized. For a hands-on walkthrough, see [Use Queues](/0-10-0/how-to/use-queues).
...
<Info title="Trigger actions">
  Named queues use the `Enqueue` trigger action. For a full comparison of synchronous, Void, and Enqueue invocation modes, see [Trigger Actions](/0-10-0/how-to/trigger-actions).
</Info>
...
<Info title="Why so many resources?">
  Each named queue creates six RabbitMQ objects to support delayed retry and dead-lettering. For the design rationale, see [Queue Architecture](/0-10-0/modules/module-queue#queue-flow).
</Info>
...
<Info title="Choosing an adapter">
  For practical guidance on which adapter to use in different deployment scenarios, see [Use Queues — Choosing an Adapter](/0-10-0/how-to/use-queues#choosing-an-adapter).
</Info>
...
<Info title="DLQ operations">
  For a complete guide on inspecting DLQ messages before redriving, see [Manage Failed Triggers](/0-10-0/how-to/dead-letter-queues).
</Info>
