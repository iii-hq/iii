...
<Info title="Trigger types">
  The available trigger types depend on which modules are loaded in your engine configuration. See [Trigger Types](/0-10-0/how-to/use-functions-and-triggers#trigger-types) and [Modules](/0-10-0/modules/index) for the full list.
</Info>
...
<Info title="State persistence">
  State persistence depends on your engine's State module configuration. With `in_memory` storage, state is lost on engine restart. With `file_based` or `RedisAdapter`, state persists across restarts. See [State Module](/0-10-0/modules/module-state) for configuration details.
</Info>
...
<Info title="Stream module required">
  Streams are provided by the Stream module. Make sure `modules::stream::StreamModule` is included in your engine configuration. See [Stream Module](/0-10-0/modules/module-stream) for details.
</Info>
...
<Info title="Observability module required">
  Trace collection requires the Observability module with `exporter` set to `memory` or `both`. See [Observability Module](/0-10-0/modules/module-observability) for configuration.
</Info>
...
<Info title="Observability module required">
  Log collection requires the Observability module with `logs_enabled: true`. See [Observability Module](/0-10-0/modules/module-observability) for configuration.
</Info>
