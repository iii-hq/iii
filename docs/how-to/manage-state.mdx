...
<Info title="Production adapters">
  The default `file_based` adapter works for single-instance use. For production with multiple replicas, use a persistent adapter like Redis. See the [State worker reference](/docs/workers/iii-state).
</Info>
