...
Use the State worker for lightweight key-value storage when you don't need a full database. Supports scopes for isolation and adapters for persistence (in-memory, file, Redis). For triggering functions on state changes, see [React to State Changes](/how-to/react-to-state-changes).

## Steps

### Enable the State worker
