...
description: 'How to read, write, and update shared state across functions (State worker `iii-state`).'
---

## Goal

Use the State worker (`iii-state`) to share data between Functions without a separate database.

## Steps

### 1. Enable the State worker (`iii-state`)
