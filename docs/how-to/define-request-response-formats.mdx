...
<Info title="Optional payload for list">
  `engine::functions::list` accepts an optional `include_internal` flag. Pass `{"include_internal": true}` if you need internal functions in the listing. The default is `false`.
</Info>
