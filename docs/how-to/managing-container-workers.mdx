...
Container workers run inside libkrun microVMs. The `iii worker` launcher injects `III_ISOLATION=libkrun` into the VM environment, and the worker's SDK forwards it on registration, so each one shows up with a purple **LIBKRUN** badge on the [console Workers page](/docs/console#workers). The launcher always wins: if an image bakes `III_ISOLATION=docker` into its env directives, the launcher overrides it to `libkrun` so the badge reflects the actual runtime.
