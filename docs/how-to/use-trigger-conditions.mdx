...
Use conditions when you need to filter trigger invocations based on business rules (e.g., only process high-value orders, skip duplicate events, check feature flags). The condition function runs before the handler and gates execution.

## Steps

### Register the handler and condition Functions
...
### Register triggers with the correct condition key
