...
iii.register_function((
    RegisterFunctionMessage::with_id("my-project::on-trigger-reg".to_string()),
    |input: Value| async move {
        let function_id = input["function_id"].as_str().unwrap_or("");
        let role = input["context"]["role"].as_str().unwrap_or("");

        let prefix = format!("{role}::");
        Ok(json!(function_id.starts_with(&prefix)))
    },
));

iii.register_function((
    RegisterFunctionMessage::with_id("my-project::on-trigger-type-reg".to_string()),
    |input: Value| async move {
        let role = input["context"]["role"].as_str().unwrap_or("");
        Ok(json!(role == "admin"))
    },
