---
title: 'Worker RBAC'
description: 'Configure the worker manager (`iii-worker-manager`) to enable role-based access control for workers connecting over WebSocket.'
---

This guide walks you through enabling RBAC on your iii instance using the **worker manager** (`iii-worker-manager`). Workers connect via WebSocket to a dedicated RBAC port, separate from the internal engine bridge.
