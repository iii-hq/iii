...
Use middleware for cross-cutting concerns like authentication, rate limiting, logging, or request validation. Middleware runs before the handler and can block requests early. For worker-level RBAC, see [Worker RBAC](/how-to/worker-rbac).
...
### Attach middleware to the trigger
