...
Use queues to run work asynchronously with built-in retries, dead-letter support, and concurrency control. iii offers **topic-based** queues (pub/sub fan-out) and **named queues** (direct function targeting). For background on when and why to use queues, see [Queue Architecture](/architecture/queues). For the full configuration reference, see the [Queue module](/modules/module-queue).
