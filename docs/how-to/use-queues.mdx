...
Process work asynchronously with built-in retries, dead-letter support, and concurrency control using topic-based queues (pub/sub fan-out) or named queues (direct function targeting). For background on when and why to use queues, see [Queue Architecture](/architecture/queues). For the full configuration reference, see the [Queue module](/modules/module-queue).

<Info title="Trigger actions primer">
  Named queues use the `Enqueue` trigger action. If you are new to trigger actions, read [Trigger Actions](./trigger-actions) first.
</Info>
...
<Info title="Full configuration reference">
  See the [Queue module reference](/modules/module-queue#configuration) for every field, adapter option, and default value.
</Info>

This guide covers two independent queue modes — pick the one that fits your use case, or use both. For help deciding, see [When to use which](/modules/module-queue#when-to-use-which).
...
<Steps>
  <Step title="Register consumers for a topic">
    ...
    iii.register_function({"id": "notify::email"}, send_email_notification)
    iii.register_function({"id": "audit::log"}, write_audit)
...
Enqueue jobs to a specific function by name with configurable retries, concurrency limits, FIFO ordering, and dead-letter support — all defined centrally in `iii-config.yaml`.
...
iii.register_function({"id": "notify::email"}, send_email_notification)
iii.register_function({"id": "inventory::reserve"}, reserve_inventory)
iii.register_function({"id": "analytics::track"}, track_analytics)
