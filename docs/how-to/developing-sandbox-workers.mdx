...
Workers started with `iii worker dev` (or any `iii worker` command that launches a libkrun VM) have `III_ISOLATION=libkrun` injected into their environment by the launcher, and the SDK forwards that value when it registers with the engine. You'll see a purple **LIBKRUN** badge next to the worker in the [console Workers page](/docs/console#workers). The badge is descriptive (what the worker reports about its environment), not an attestation — direct SDK connections can override the value by exporting `III_ISOLATION` themselves.
