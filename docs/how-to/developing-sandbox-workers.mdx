...
| `runtime.base_image` | No | language default | OCI image used as the sandbox rootfs. Overrides the per-language default (e.g. `oven/bun:1` to run TypeScript on bun). See [Custom base images](#custom-base-images) below. |
...
By default iii picks a rootfs image per language: `iiidev/node:latest` for TypeScript/JavaScript, `iiidev/python:latest` for Python, `library/rust:slim-bookworm` for Rust. Override with `runtime.base_image` when you need a different runtime — a common case is running TypeScript on bun instead of node:
...
Each distinct `base_image` gets its own cache directory under `~/.iii/rootfs/<slug>/`, so adding a bun-based worker won't clobber the shared node rootfs used by other TypeScript workers. The image is pulled on first start and reused for every subsequent start that names the same image.

<Info title="Memory-hungry runtimes get real disk-backed swap">
  Local-path sandboxes automatically attach a sparse 2 GiB swap image (`~/.iii/managed/<worker>/swap.img`) as a virtio-blk device. iii-init runs `mkswap` + `swapon` at boot and widens the worker cgroup's `memory.swap.max` to match `memory.max`. This fixes the "bun OOM-kills inside the microVM" problem — bun's allocator maps ~73 GiB virtual and ignores cgroup v2 RAM limits, so without real disk swap the kernel kills it. The swap file is sparse, so the host pays zero bytes until the guest actually swaps pages out, and swapped pages survive VM reboots (the signature is detected and `mkswap` is skipped on re-start).
</Info>
