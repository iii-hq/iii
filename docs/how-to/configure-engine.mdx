...
- **Workers** — Engine capabilities (HTTP, queues, state, cron, observability, etc.) listed under `workers:` in YAML
...
<ResponseField name="workers" type="array" required>
  List of workers to enable. Each entry requires a `name` identifier (for example `iii-http`, `iii-queue`) and an optional `config` object.
</ResponseField>
...
<Accordion title="HTTP worker (`iii-http`)">
...
<Accordion title="Stream worker (`iii-stream`)">
...
<Accordion title="State worker (`iii-state`)">
...
<Accordion title="Queue worker (`iii-queue`)">
...
<Accordion title="PubSub worker (`iii-pubsub`)">
...
<Accordion title="Cron worker (`iii-cron`)">
...
<ResponseField name="config.adapter" type="object" required>
  ...
  <Expandable title="KV-based cron adapter">
    **Name:** `kv`
    ...
    <Warning>
      Local locks only prevent duplicates within the same process. Multiple engine instances may execute the same cron job. Use the `redis` adapter for true distributed locking.
    </Warning>
...
<Accordion title="Observability worker (`iii-observability`)">
...
<Accordion title="HTTP Functions worker (`iii-http-functions`)">
...
<Accordion title="Exec worker (`iii-exec`)">
...
<Accordion title="Bridge client worker (`iii-bridge`)">
...
<Accordion title="Telemetry worker (`iii-telemetry`)">
