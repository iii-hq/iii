...
Use `iii trigger` for development testing, one-off operations, CI/CD scripts, or redriving dead-letter queue messages. The CLI connects to the engine via WebSocket, sends the invocation, and prints the result. For programmatic invocation, use the SDK's `trigger()` method instead.
