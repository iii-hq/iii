...
<Steps>
  ...
  <Step title="Identify the function ID">
    ...
    - `durable::publish` — the builtin topic-based publish function
  </Step>
...
Use the builtin `durable::publish` function to publish a message to a topic:

```bash
iii trigger \
  --function-id='durable::publish' \
