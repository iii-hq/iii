...
<Steps>
  ...
  <Step title="Identify the function path">
    Every function registered with the engine has a unique path. Builtin functions use the `iii::` prefix. User-defined functions use the ID you specified during registration.
...
Use the builtin `iii::durable::publish` function to publish a message to a topic. The nested `data` object makes `--json` the cleanest form:
