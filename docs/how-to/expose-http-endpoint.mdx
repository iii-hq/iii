...
Use this when you need to expose iii functions to HTTP clients (web apps, mobile apps, external services). Requires the `iii-http` worker. For WebSocket-based clients, see [Use iii in the Browser](/how-to/use-iii-in-the-browser).

## Steps

### Enable the REST API worker
