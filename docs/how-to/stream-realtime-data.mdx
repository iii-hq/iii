...
Use streams when clients need live updates without polling. Clients connect to the Stream worker via WebSocket and subscribe to groups (rooms, channels, user IDs). For binary data transfer between workers, see [Use Channels](/how-to/use-channels) instead.

## Steps

### Enable the Stream worker
