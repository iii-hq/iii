...
Enqueue jobs to a specific function by name with configurable retries, concurrency limits, FIFO ordering, and dead-letter support. All named queues are defined centrally in `iii-config.yaml`. For help deciding between named and topic-based queues, see [When to use which](/modules/module-queue#when-to-use-which).
...
<Steps>
  ...
  <Step title="Enqueue work via trigger action">
    ...
    const receipt = await iii.trigger({
      ...
      payload: { orderId: 'ord_789', amount: 149.99, currency: 'USD', transaction_id: 'txn-001' },
  ...
  <Step title="Handle the enqueue result">
    ...
    try {
      const receipt = await iii.trigger({
        function_id: 'orders::process-payment',
        payload: { orderId: 'ord_789', amount: 149.99, transaction_id: 'txn-001' },
        action: TriggerAction.Enqueue({ queue: 'payment' }),
      })
    ...
    match iii.trigger(TriggerRequest {
        function_id: "orders::process-payment".to_string(),
        payload: json!({ "orderId": "ord_789", "amount": 149.99, "transaction_id": "txn-001" }),
        action: Some(TriggerAction::Enqueue { queue: "payment".to_string() }),
        timeout_ms: None,
    }).await {
