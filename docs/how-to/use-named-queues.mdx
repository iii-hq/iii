...
Enqueue jobs to a specific function by name with configurable retries, concurrency limits, FIFO ordering, and dead-letter support.

## Context

Use named queues when you need reliable async processing with retries and dead-letter support. Jobs are routed to a specific function defined in the queue config. For fan-out to multiple subscribers, see [Use Topic Queues](/how-to/use-topic-queues). For understanding trigger actions, see [Trigger Actions](/primitives-and-concepts/trigger-actions).

## Steps

### 1. Enable the Queue worker
