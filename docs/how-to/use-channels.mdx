...
Stream binary data between functions that may run in different worker processes using Node.js-style readable/writable streams.

## Context

Use channels when you need to transfer large files, stream HTTP responses progressively, or implement bidirectional communication between functions. Channels work across process and language boundaries. For pushing updates to connected clients, see [Stream Real-Time Data](/how-to/stream-realtime-data) instead. For channel architecture details, see [Architecture: Channels](/architecture/channels).
