...
For binary workers, the command also checks the lockfile target triple. A lockfile generated on macOS with an `aarch64-apple-darwin` binary pin will fail verification on Linux CI until the worker is resolved for the Linux target.
