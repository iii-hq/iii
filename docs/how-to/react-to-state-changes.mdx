...
Use state triggers for reactive patterns like cache invalidation, audit logging, or notification dispatch when data changes. Requires the `iii-state` worker. For basic state operations (get/set/delete), see [Manage State](/how-to/manage-state).
