...
Use topic queues for event-driven fan-out where multiple systems need to react to the same event (e.g., order placed triggers email, inventory, and analytics). Each subscriber gets its own copy with independent retries. For point-to-point queues, see [Use Named Queues](/how-to/use-named-queues).
