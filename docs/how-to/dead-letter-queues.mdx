...
Use DLQs when processing critical work (payments, emails, webhooks) where you need visibility into failures and the ability to manually retry. Requires the `iii-queue` worker with named queues configured. See [Use Named Queues](/how-to/use-named-queues) for queue setup.

## Steps

### Register the external endpoint as an HTTP-invoked function
...
### Define a named queue with retry configuration
...
### What happens when a job lands in the DLQ
