...
<Tabs>
<Tab title="RabbitMQ Management UI">
  The Management UI supports moving messages between queues.

  <Steps>
    <Step title="Navigate to the DLQ queue">
      Open the **Queues and Streams** tab and click on the DLQ queue (e.g. `iii.__fn_queue::payment::dlq.queue`).
    </Step>
    <Step title="Move messages">
      Expand the **Move messages** section. Set the **Destination queue** to the main queue:

      ```
      iii.__fn_queue::payment.queue
      ```

      Click **Move messages**. RabbitMQ moves all messages from the DLQ back to the main queue using the Shovel plugin.

      <Info title="Shovel plugin required">
        The "Move messages" feature requires the `rabbitmq_shovel` plugin. Enable it with `rabbitmq-plugins enable rabbitmq_shovel rabbitmq_shovel_management`.
      </Info>
    </Step>
    <Step title="Verify">
      Check the DLQ queue — it should now be empty. The main queue message count should have increased by the number of redriven messages.
    </Step>
  </Steps>
</Tab>
<Tab title="rabbitmqadmin CLI">
  You can redrive messages by consuming from the DLQ and republishing to the main exchange.

  **Redrive all messages from a DLQ:**

  ```bash
  # Consume one message from the DLQ and republish it to the main exchange.
  # Repeat until the DLQ is empty.
  while true; do
    msg=$(rabbitmqadmin get queue="iii.__fn_queue::payment::dlq.queue" count=1 ackmode=ack_requeue_false --format=raw_json)

    # Exit when the DLQ is empty
    if [ "$msg" = "[]" ]; then
      echo "DLQ is empty — redrive complete."
      break
    fi
    ...
    rabbitmqadmin publish \
      exchange="iii.__fn_queue::payment" \
      routing_key="payment" \
      payload="$payload"

    echo "Redrove 1 message"
  done
  ```

  Replace `payment` with your queue name in both the DLQ queue name and the target exchange/routing key.

  <Warning title="Redrive only after fixing the root cause">
    Redriving messages without fixing the underlying issue sends them through the same retry cycle, and they will end up in the DLQ again.
  </Warning>
</Tab>
<Tab title="Builtin adapter">
  The builtin adapter has an internal redrive mechanism that pops all messages from the DLQ, resets their attempt counters, and pushes them back to the main queue.

  This capability is not currently exposed via CLI or REST API. For workflows that require DLQ redrive, use the [RabbitMQ adapter](/modules/module-queue#modulesqueuerabbitmqadapter) in production.
</Tab>
</Tabs>
