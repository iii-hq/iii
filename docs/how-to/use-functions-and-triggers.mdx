...
<Info title="Queue triggers vs named queues">
  Use a `queue` trigger when you want a Function to consume a topic such as `user.created`. Publish to that topic via the built-in `enqueue` function.

  Named queues are separate: use `TriggerAction.Enqueue({ queue: 'name' })` to enqueue a direct Function invocation with retries and queue-level configuration. That flow does not register a `queue` trigger.

  See [Use Queues](/how-to/use-queues), [Trigger Actions](/how-to/trigger-actions), and the [Queue module](/modules/module-queue) for the full model.
</Info>
