...
{/* <Info title="Queue uses durable::publish">
  Queue messaging uses `durable::publish` — a built-in function, no trigger registration needed:
  Node `trigger({ function_id: 'durable::publish', payload: { topic: 'user.created', data: {...} } })`,
  Python `trigger({'function_id': 'durable::publish', 'payload': {'topic': 'user.created', 'data': {...}}})`,
  Rust `trigger(TriggerRequest::new("durable::publish", json!({"topic": "user.created", "data": {...}})))`.
...
| Function | Purpose | Parameters |
|----------|---------|------------|
| `durable::publish` | Publish a message | `topic`, `data` |
