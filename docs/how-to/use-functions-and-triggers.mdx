...
description: 'Register functions, bind them to triggers, and invoke them across language boundaries.'
---

## Goal

Register functions with the engine and trigger them from other functions, workers, or external events.

## Context

Use this guide when building backend logic with iii. Functions are the unit of work; triggers are the events that invoke them. For conceptual background, see [Primitives: Functions, Triggers, Workers](/primitives-and-concepts/functions-triggers-workers). For trigger type configuration details, see [Triggers](/architecture/trigger-types).

## Register and Trigger a Function
...
See the [Engine Configuration Reference](/api-reference/engine-config) for details.
</Warning>
...
For a complete list of trigger types and their configuration options, see [Triggers](/architecture/trigger-types).
</Info>
...
The engine provides built-in functions for state, streams, queues, pubsub, and observability. Trigger them like any other function.

<Info title="Built-in Functions Reference">
For the complete list of built-in functions and their parameters, see [Built-in Functions Reference](/api-reference/built-in-functions).
</Info>
