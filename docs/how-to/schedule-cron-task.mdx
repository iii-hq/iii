...
description: 'How to run a Function on a recurring schedule with the Cron worker (`iii-cron`).'
---

## Goal

Run a Function at regular intervals (cleanup jobs, report generation, health checks) using the Cron worker (`iii-cron`).

## Steps

### 1. Enable the Cron worker (`iii-cron`)
