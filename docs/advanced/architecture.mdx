...
description: 'An overview of the iii system components — engine core, built-in workers, and worker processes — and how they interact.'
---
...
2. **Built-in Workers** - Provide interfaces to external systems (HTTP, Redis, WebSocket)
3. **Worker Processes** - External processes that execute business logic
...
Built-in workers implement the `Worker` trait and bridge external protocols to internal function calls.
...
- **Rust Performance**: Built-in workers built for speed and memory efficiency
...
Build custom built-in workers by implementing the `Worker` trait:
