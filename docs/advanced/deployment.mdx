...
The engine uses a YAML configuration file to define active workers (engine capabilities) and their settings.
...
When you start the engine with `iii --use-default-config`, it loads the workers registered as enabled by default in the current build, including the core REST API, queue, cron, pub/sub, state, stream, and KV-backed capabilities.
...
3. **Distributed Locking**: The Cron worker (`iii-cron`) uses Redis locks to prevent duplicate execution
...
2. **HTTP and stream ports**: HTTP API (`iii-http`, 3111), Streams (`iii-stream`, 3112)
...
Configure the Observability worker (`iii-observability`) (traces, metrics, logs):
