...
<Info title="Trigger types">
  The available trigger types depend on which workers are enabled in your engine configuration. See [Trigger Types](/architecture/trigger-types) and [Workers](/modules) for the full list.
</Info>
...
<Info title="State persistence">
  State persistence depends on your engine's State worker configuration. With `in_memory` storage, state is lost on engine restart. With `file_based` or the `redis` adapter, state persists across restarts. See [State Worker](/modules/module-state) for configuration details.
</Info>
...
<Info title="Observability worker required">
  Trace collection requires the Observability worker (`iii-observability`) with `exporter` set to `memory` or `both`. See [Observability Worker](/modules/module-observability) for configuration.
</Info>
...
<Info title="Observability worker required">
  Log collection requires the Observability worker (`iii-observability`) with `logs_enabled: true`. See [Observability Worker](/modules/module-observability) for configuration.
</Info>
