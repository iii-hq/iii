...
<Update label="0.11.0 (next)" description="April 2026">
  ...
  **Breaking.** The Motia framework is deprecated in favor of using `iii-sdk` directly. Moving to the SDK unlocks multi-worker orchestration, browser connectivity via `iii-browser-sdk` with RBAC, and a direct understanding of iii's three primitives — Workers, Functions, and Triggers. Your existing Motia project becomes one worker in a larger iii deployment instead of a standalone monolith.
  ...
  ## Worker Isolation badge on the Workers page

  The console Workers page now surfaces an **Isolation** badge next to the runtime badge, reflecting the environment the worker is running inside. Workers launched via `iii worker dev` or `iii worker add <image>` show a purple **LIBKRUN** badge automatically — the launcher injects `III_ISOLATION=libkrun` into the VM environment and the SDK forwards it on registration. Direct SDK connections can export `III_ISOLATION=docker` (or any other value) to tag themselves for filtering and debugging. All three SDKs (Node, Python, Rust) read the env var; the engine stores it on `WorkerConnection` and exposes it via `engine::workers::list`.
