...
language-specific project directory with the iii SDK installed, a root `worker-compose.yaml`, and
...
The root `worker-compose.yaml` catalog tells iii how to build, validate, publish, and run local
workers. `iii compose validate` compiles it into immutable package descriptors; `iii worker add`
installs Registry references only. See the [worker package descriptor](./worker-manifest).

<Note>
  If a worker isn't starting correctly then run `iii compose validate` and check [`iii worker
