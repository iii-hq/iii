...
fn should_fallback_to_legacy_registry_error(name: &str, error: &str) -> bool {
    error.starts_with("Failed to parse worker graph:")
        ...
        || (error.starts_with("Failed to resolve worker graph: HTTP 422")
            && error.contains("invalid_descriptor"))
}
...
",
    ...
    fn legacy_registry_fallback_is_limited_to_compatibility_failures() {
        ...
        assert!(should_fallback_to_legacy_registry_error(
            "pdfkit",
            "Failed to resolve worker graph: HTTP 422 {\"error\":{\"code\":\"invalid_descriptor\"}}"
        ));
        assert!(!should_fallback_to_legacy_registry_error(
            "pdfkit",
            "Failed to resolve worker graph: HTTP 422 {\"error\":{\"code\":\"invalid_version\"}}"
        ));
        assert!(!should_fallback_to_legacy_registry_error(
            "pdfkit",
