...
pub fn check_libkrunfw_platform_support() -> Result<(), String> {
    ...
    if os == "macos" && arch == "x86_64" {
        return Err("libkrunfw firmware is not available for Intel Macs (darwin-x86_64).\n\
             Managed workers require an Apple Silicon Mac (aarch64) or a Linux host.\n\
             Set III_LIBKRUNFW_PATH to a manually-built firmware file to override.".to_string());
    }
    Ok(())
}
