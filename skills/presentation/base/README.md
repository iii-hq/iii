...
One Vite project that builds every tech spec into **__SITE_HOST__/tech-specs/** — a
...
linkable beside it, and `index.json` feeding the __SITE_HOST__ landing timeline.
...
URLs: `__SITE_HOST__/tech-specs/` (gallery) · `__SITE_HOST__/tech-specs/<slug>/` (deck or
viewer) · `…/<slug>/#/spec` (a deck's reading mode) · `…/<slug>/<file>.md`
(raw markdown) · `__SITE_HOST__/tech-specs/index.json` (machine-readable list).
