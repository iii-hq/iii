...
app.post("/state/carts/:cartId/items", async (req, res) => {
  ...
  await sendCentralLog("info", "state.cart_add_item", {
    cartId: req.params.cartId,
    sku: req.body.sku,
  });
  span.end();
...
app.get("/state/carts/:cartId", async (req, res) => {
  ...
  if (!cartRaw) {
    await sendCentralLog("warn", "state.cart_get.not_found", {
      cartId: req.params.cartId,
    });
  ...
  await sendCentralLog("info", "state.cart_get.found", {
    cartId: req.params.cartId,
...
app.post("/state/carts/:cartId/checkout", async (req, res) => {
  ...
  const cartRaw = await redis.get(key);
  if (!cartRaw) {
    await sendCentralLog("warn", "state.checkout.not_found", {
      cartId: req.params.cartId,
    });
    res.status(404).json({ error: "Cart not found" });
    return;
  ...
  await sendCentralLog("info", "state.checkout.completed", {
    cartId: req.params.cartId,
  });
  res.json(cart);
});
...
async function bootStateReactions() {
  ...
  await subscriber.subscribe("state.carts.changed", async (raw) => {
    const event = JSON.parse(raw) as {
      cartId: string;
      cart: { checkedOut: boolean };
    };
    if (event.cart.checkedOut) {
      await redis.incr("metrics:checkedout_carts");
      await sendCentralLog("info", "state.metrics.checkedout_incremented", {
        cartId: event.cartId,
      });
    }
  });
