...
const logger = pino({ name: "ai-agents-traditional", level: process.env.LOG_LEVEL ?? "info" });
...
async function planNode(state: AgentState): Promise<Partial<AgentState>> {
  ...
  await sendCentralLog("info", "agents.plan.completed", { sessionId: state.sessionId, needsDocs });
  return { needsDocs };
}
...
async function retrieveNode(state: AgentState): Promise<Partial<AgentState>> {
  ...
  const docs = stored.length > 0 ? stored : ["runbook", "api contract", "support policy"];
  await sendCentralLog("info", "agents.retrieve.completed", { sessionId: state.sessionId, count: docs.length });
  return { docs };
}
...
async function respondNode(state: AgentState): Promise<Partial<AgentState>> {
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      ...
      { role: "user", content: `${state.memory.join("\n")}\n${state.docs.join("\n")}\n${state.input}`.trim() },
    ],
    stream: true,
  });
  let answer = "";
  ...
  await sendCentralLog("info", "agents.respond.completed", { sessionId: state.sessionId, outputLength: answer.length });
  return { answer };
}
...
async function persistNode(state: AgentState): Promise<Partial<AgentState>> {
  ...
  await sendCentralLog("info", "agents.persist.completed", { sessionId: state.sessionId });
  return {};
}
...
const graph = new StateGraph<AgentState>({ channels: {} as any })
  ...
  .addConditionalEdges("plan", (state) => (state.needsDocs ? "retrieve" : "respond"))
...
app.post("/agents/chat", async (req, res) => {
  ...
  await sendCentralLog("info", "agents.chat.completed", { sessionId });
  span.end();
