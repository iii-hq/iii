...
const logger = pino({ name: "logging-traditional", level: process.env.LOG_LEVEL ?? "info" });
...
app.post("/checkout", async (req, res) => {
  ...
  await sendCentralLog("info", "checkout.request.received", { requestId, cartId: req.body.cartId });
  ...
  pricingSpan.end();
  await sendCentralLog("info", "checkout.calculate_totals.done", { requestId, subtotal, tax, total });
  ...
  if (!approved) {
    await sendCentralLog("warn", "checkout.charge_payment.declined", { requestId });
    rootSpan.end();
    res.status(402).json({ error: "Payment declined" });
    return;
  }

  await sendCentralLog("info", "checkout.completed", { requestId, total });
  rootSpan.end();
