...
iii.registerFunction({ id: "checkout::charge-payment" }, async (data: any) => {
  ...
  if (!approved) {
    logger.warn("checkout.charge_payment.declined", {
      requestId: data.requestId,
    });
    const error = new Error("Payment declined") as Error & {
      status: number;
    };
    error.status = 402;
    throw error;
  }
  logger.info("checkout.charge_payment.approved", {
    requestId: data.requestId,
    total: data.total,
  });
