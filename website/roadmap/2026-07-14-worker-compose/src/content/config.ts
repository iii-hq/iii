...
export const CONFIG_RULES = [
  {
    ...
    desc: "config_name resolves through the configuration worker; storage is an adapter (fs, secrets manager, another iii). file paths are not the contract.",
  },
  {
    name: "fetch or fail",
    desc: "a container whose base fetch fails does not start — up fails before a process exists rather than booting on wrong defaults.",
  },
  { name: "merge semantics", desc: "maps merge per key; arrays and scalars replace; null is an explicit value." },
  {
    name: "why values, not a pointer",
    desc: '--config is a first-boot seed today ("thereafter the configuration worker is the authoritative source"). delivering resolved values keeps the compose file honest on every restart.',
  },
] as const
