...
export const SCRIPT_STAGES: RevealStage[] = [
  {
    ...
    caption:
      "base fetched from the configuration worker, config_override merged. failure here means the container never starts.",
    rows: [
  ...
  },
  {
    label: "pre_start",
    caption:
      "npx prisma migrate deploy — blocking, in its own process group, 60s default budget (pre_start_timeout to change).",
    rows: [
  ...
  },
  {
    label: "spawn run",
    caption:
      "path:// worker: the compose run command. package:// binary: exec the artifact with --url --namespace --config.",
    rows: [
  ...
  },
  {
    ...
    tone: "warn",
    caption:
      "any exit path — down, a post-ready crash, or an up rollback. exit is confirmed before anything else happens.",
    rows: [
      { k: "status", v: "exited" },
  ...
  },
  {
    label: "post_run",
    caption:
      "./cleanup-tmp.sh — fired once after the exit is confirmed, never awaited. a non-zero exit is a warning, not a blocker.",
    rows: [
...
export const SCRIPT_FIELDS = [
  ...
  {
    name: "pre_start_timeout",
    desc: "default 60s; set the field only to change it; timeout kills the hook’s process group.",
  },
  {
    name: "run",
    desc: "the supervised process. path:// only — required when the worker has no manifest. compose run wins over manifest scripts.start.",
  },
  {
    name: "post_run",
    desc: "fired once after the run process exits — any exit path (down, crash, rollback); not awaited; failure = warning + last_error.",
  },
] as const
