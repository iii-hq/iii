...
export const PAYOFF_SOLVES = [
  {
    ...
    detail:
      "injected renderers dispatch first; null falls through to the 13 built-in families, so overriding a built-in is just matching its ids.",
  },
  {
    problem: "the memory page touched 26 console files, 23 under web/src",
    answer: "a page is one script asset + one trigger",
    detail:
      "host.pages.register mounts #/ext/<id>; presence-gating comes free because the trigger dies with the worker.",
  },
  {
    ...
    detail:
      "the null-schema branch — where SchemaForm cannot render — is exactly where a custom form is worth the most. persistence stays host-owned.",
  },
  {
