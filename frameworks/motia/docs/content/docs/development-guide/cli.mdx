...
The iii engine is the runtime that manages all infrastructure workers (queues, state, streams, cron, HTTP) and your Motia application process.
...
<Callout type="info">
During development, your `package.json` should have `"dev": "iii"` so that `npm run dev` starts the engine, which reads `config.yaml` and boots all configured workers including your Motia application via the iii-exec worker.
</Callout>
...
Your `config.yaml` should reference the production bundle in the iii-exec worker (place this under the top-level `workers:` list):

```yaml title="config.yaml (production iii-exec)"
workers:
...
Start the Motia SDK in development mode. This is typically run automatically by the iii engine's **iii-exec** worker, not manually.

```bash
npx motia dev
```

Your `config.yaml` iii-exec worker for development (place this under the top-level `workers:` list). The `watch` array defines file patterns that trigger a rebuild, and the `exec` commands run sequentially — `npx motia dev` compiles the Steps, then `node dist/index-dev.js` starts the built output:

```yaml title="config.yaml (development iii-exec)"
workers:
  # ... other workers (iii-stream, iii-state, etc.) ...
...
The `iii` engine reads `config.yaml`, starts all workers, and uses the iii-exec worker to build and run your Motia Steps automatically. File changes trigger hot-reload.
