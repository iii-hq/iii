...
Create a production `config.yaml` with Redis adapters for all workers that need them. Configure `REDIS_URL` to point at Railway's private Redis endpoint:
...
**Fix:** Make sure your `config.yaml` uses `${PORT:3111}` for the iii-http worker port so Railway can inject its port:
