...
<Callout type="warn">
This Docker recipe is for JavaScript/TypeScript projects built with `motia build` (Bun runtime image). For Python-only or mixed projects, use the Python runtime pattern from the [Docker guide](/docs/deployment-guide/docker#python-steps) and include a Python iii-exec worker alongside the Node/Bun runtime as needed.
</Callout>
...
The iii engine is the entrypoint for your Motia app in production. It reads `config.yaml` and manages all workers. Make sure your `config.yaml` binds to `0.0.0.0` -- Fly needs your app to listen on all interfaces.

Use `${PORT:3111}` in your iii-http worker config so Fly can inject its port if needed.

### Configure Redis

Create a production `config.yaml` with Redis adapters for all workers that need them. Fly sets `REDIS_URL` when you run `flyctl redis create`:
...
**Fix:** Make sure your `config.yaml` iii-http worker uses `host: 0.0.0.0` and the port matches `fly.toml`'s `internal_port`:
