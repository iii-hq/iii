...
## JavaScript module system and runtime
...
| Configuration | Shared with Node steps | Own `config.yaml` iii-exec worker entry |

### For Mixed Projects (Node + Python)

Configure **separate iii-exec worker entries** in `config.yaml`:
