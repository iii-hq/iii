...
<Tabs items={['cron', 'checker', 'alerter', 'health', 'utilities']}>
  <Tab value="cron">
    ...
    import { cron, logger, enqueue } from 'motia'
    ...
    export async function handler() {
      logger.info(`Starting uptime checks for ${envConfig.sites.length} sites`);
      logger.info(`Sites configured: ${JSON.stringify(envConfig.sites)}`);

      try {
        for (const url of envConfig.sites) {
          logger.info(`Scheduling check for: ${url}`);
          ...
          logger.info(`Successfully enqueued for: ${url}`);
        }

        logger.info(`Successfully scheduled checks for all ${envConfig.sites.length} sites`);
      } catch (error) {
