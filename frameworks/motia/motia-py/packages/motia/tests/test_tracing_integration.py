...
async def test_enqueue_creates_child_span(otel_exporter, patch_motia_bridge, api_url):
    ...
    enqueue_spans = [s for s in spans if s.name == "iii::durable::publish"]
    assert len(enqueue_spans) >= 1, f"Expected at least one 'iii::durable::publish' span, got: {[s.name for s in spans]}"
