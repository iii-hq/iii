...
use iii::function::{FunctionResult, FunctionHandler};
use iii::protocol::ErrorBody;
use iii::modules::event::{EventAdapter, SubscriberQueueConfig};

#[cfg(feature = "rabbitmq")]
use iii::modules::event::adapters::rabbitmq::{RabbitMQAdapter, types::{RabbitMQConfig, QueueMode}};
                ...
                if *count < self.max_failures {
                    *count += 1;
                    return FunctionResult::Failure(ErrorBody {
                        ...
                        message: format!("Simulated failure {}/{} for job {}", *count, self.max_failures, job_id),
                    });
                }
            }
...
async fn setup_adapter_with_config(engine: Arc<Engine>, mut config: RabbitMQConfig) -> Arc<dyn EventAdapter> {
    config.amqp_url = TEST_AMQP_URL.to_string();
    let adapter = RabbitMQAdapter::new(config, engine).await.expect("Failed to create adapter");
...
async fn test_emit_and_subscribe() {
    ...
    adapter.subscribe(&topic, "test-sub-1", &function_path, None, None).await;
...
async fn test_multiple_subscribers() {
    ...
    adapter.subscribe(&topic, "test-sub-1", &function_path1, None, None).await;
    adapter.subscribe(&topic, "test-sub-2", &function_path2, None, None).await;
    ...
    assert_eq!(received1_msgs.len() + received2_msgs.len(), 1, "Exactly one subscriber should receive the message");
...
async fn test_subscribe_unsubscribe() {
    ...
    let received_msgs = received.lock().await;
    assert_eq!(received_msgs.len(), 1, "Should only receive message before unsubscribe");
...
async fn test_fifo_mode_ordering() {
    ...
    adapter.subscribe(&topic, "test-sub-1", &function_path, None, Some(queue_config)).await;
    sleep(Duration::from_millis(500)).await;
...
async fn test_retry_on_failure() {
    ...
    let received_msgs = received.lock().await;
    assert_eq!(received_msgs.len(), 1, "Should eventually succeed after immediate retries");
...
async fn test_max_attempts_dlq() {
    ...
    let final_count = adapter.dlq_count(&topic).await.expect("Failed to get DLQ count");
    assert_eq!(final_count - initial_count, 1, "Message should be in DLQ after max attempts");
...
async fn test_dlq_count() {
    ...
    let final_count = adapter.dlq_count(&topic).await.expect("Failed to get final DLQ count");
    assert_eq!(final_count - initial_count, 2, "Should have 2 messages in DLQ");
...
async fn test_redrive_dlq() {
    ...
    let dlq_count_before = adapter.dlq_count(&topic).await.expect("Failed to get DLQ count");
    assert!(dlq_count_before > 0, "Should have messages in DLQ");
    
    let redriven = adapter.redrive_dlq(&topic).await.expect("Failed to redrive DLQ");
    assert_eq!(redriven, dlq_count_before, "Should redrive all DLQ messages");
    
    sleep(Duration::from_millis(2000)).await;
    
    let dlq_count_after = adapter.dlq_count(&topic).await.expect("Failed to get DLQ count after redrive");
    
    assert!(dlq_count_after >= dlq_count_before, "Messages should be back in DLQ after failing again");
