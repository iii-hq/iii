...
describe('trigger registration error surfacing', () => {
  ...
  it('logs to console.error on TriggerRegistrationResult with error', async () => {
    ...
    sock.send(
      JSON.stringify({
        ...
        error: {
          ...
          message:
            'Trigger type "http" not found — worker http is missing. Declare package://http@next in worker-compose.yaml and run iii compose --up',
        },
      }),
    )
    ...
    expect(formatted).toContain('package://http@next')
    spy.mockRestore()
  })
