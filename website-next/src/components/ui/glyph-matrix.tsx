...
interface GlyphMatrixProps extends React.HTMLAttributes<HTMLCanvasElement> {
  ...
  color?: string
  /** CSS font-family for the glyphs */
  fontFamily?: string
}
...
export function GlyphMatrix({
  ...
  // Read on each frame rather than captured by the effect, so flipping them
  // (scrolling in and out, the site font resolving) never regenerates the grid.
  ...
  useEffect(() => {
    ...
    let cells: string[] = []
    let alphas: number[] = []
    // Where each cell's alpha is heading. Mutations set the target; every
    // frame eases toward it, so glyphs breathe in and out instead of popping.
    let targets: number[] = []
    let raf = 0
    let last = 0
    let stopped = false
    ...
    const draw = () => {
      ...
      ctx.font = `${cellSize - 2}px ${fontRef.current}`
    ...
    const tick = (t: number) => {
      ...
      if (t - last >= intervalRef.current) {
        ...
        for (let n = 0; n < mutations; n++) {
          ...
          targets[i] = 0.05 + Math.random() * 0.45
        }
      }

      // Ease every cell toward its target (~12% per frame ≈ 300ms settle) and
      // redraw each frame so the field moves continuously rather than in steps.
      let moving = false
      for (let i = 0; i < alphas.length; i++) {
        const d = targets[i] - alphas[i]
        if (Math.abs(d) > 0.002) {
          alphas[i] += d * 0.12
          moving = true
        } else if (d !== 0) {
  ...
  }, [glyphs, cellSize, mutationRate, fadeBottom])

  return (
    // biome-ignore lint/a11y/noAriaHiddenOnFocusable: a canvas without tabIndex is not focusable; it's pure decoration
    <canvas
      ref={canvasRef}
