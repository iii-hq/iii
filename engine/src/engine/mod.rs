...
impl EngineTrait for Engine {
    ...
    async fn call(
        ...
        self.call_with_timeout(function_id, input, None).await
    }
...
mod tests {
    ...
    async fn call_with_timeout_evicts_deferred_invocation_on_elapse() {
        ...
        let function = crate::function::Function {
            handler: Arc::new(move |invocation_id: Option<uuid::Uuid>, _input, _session| {
                let seen = seen.clone();
                Box::pin(async move {
        ...
        let result = engine
            .call_with_timeout("test::deferred", json!({}), Some(Duration::from_millis(50)))
            .await;
    ...
    async fn call_with_timeout_none_does_not_bound_a_completing_handler() {
        ...
        let result = engine
            .call_with_timeout("test::slow", json!({}), None)
            .await;
        assert!(
            result.is_ok(),
