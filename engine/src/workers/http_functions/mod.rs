...
mod tests {
    ...
    fn build_worker(engine: Arc<crate::engine::Engine>) -> HttpFunctionsWorker {
    ...
    async fn create_and_initialize_registers_http_functions_service() {
        ...
        let worker = HttpFunctionsWorker::create(engine.clone(), Some(config))
            .await
            .expect("create http functions worker");
        assert_eq!(worker.name(), "HttpFunctionsWorker");

        worker.initialize().await.expect("initialize worker");
        ...
        assert!(Arc::strong_count(worker.http_invoker()) >= 1);
        assert!(worker.http_functions().is_empty());
    ...
    async fn register_http_function_invokes_local_server_and_unregisters() {
        ensure_default_meter();
        let engine = Arc::new(crate::engine::Engine::new());
        let worker = build_worker(engine.clone());
        ...
        assert!(worker.http_functions.contains_key("remote.echo"));
    ...
    async fn register_http_function_rejects_invalid_url() {
        ...
        assert!(!worker.http_functions.contains_key("remote.bad"));
    }
