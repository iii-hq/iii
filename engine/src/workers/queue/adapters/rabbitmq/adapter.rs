...
fn subscription_topic(key: &str) -> Option<&str> {
    key.rsplit_once(':').map(|(topic, _)| topic)
}
...
impl QueueAdapter for RabbitMQAdapter {
    ...
    async fn topic_stats(&self, topic: &str) -> anyhow::Result<crate::workers::queue::TopicStats> {
        ...
        let consumer_count = {
            ...
            subs.keys()
                .filter(|key| subscription_topic(key).map(|t| t == topic).unwrap_or(false))
                .count() as u64
        };
    ...
    async fn list_topics(&self) -> anyhow::Result<Vec<crate::workers::queue::TopicInfo>> {
        ...
        for key in subs.keys() {
            if let Some(topic) = subscription_topic(key) {
