...
use crate::{
    ...
    workers::module::{AdapterFactory, ConfigurableWorker, Worker},
    protocol::ErrorBody,
...
impl Worker for QueueWorker {
    ...
    async fn initialize(&self) -> anyhow::Result<()> {
        tracing::info!("Initializing QueueWorker");
        self._config.validate()?;
        self.engine.set_queue_worker(Arc::new(self.clone())).await;
        ...
        let trigger_type =
            TriggerType::new("queue", "Queue core worker", Box::new(self.clone()), None);
...
mod tests {
    use super::*;
    use crate::workers::module::Worker;
    use serde_json::json;
    ...
    // QueueWorker::name
    // =========================================================================
    // Note: The worker itself requires an adapter and engine, so we test what
    ...
    fn setup_queue_worker() -> (Arc<Engine>, QueueWorker, Arc<MockQueueAdapter>) {
    ...
    #[test]
    fn queue_worker_name() {
        let (_engine, worker, _adapter) = setup_queue_worker();
        assert_eq!(Worker::name(&worker), "QueueWorker");
    }
    ...
    async fn enqueue_with_empty_topic_returns_failure() {
        ...
        let result = worker.enqueue(input).await;
        match result {
    ...
    async fn enqueue_success_calls_adapter() {
        let (_engine, worker, adapter) = setup_queue_worker();
        let input = QueueInput {
    ...
    async fn redrive_with_empty_queue_returns_failure() {
        ...
        let result = worker.redrive(input).await;
        match result {
            FunctionResult::Failure(e) => {
    ...
    fn adapter_name_from_config_some() {
        let config = super::super::config::QueueWorkerConfig {
            adapter: Some(crate::workers::module::AdapterEntry {
    ...
    async fn initialize_registers_trigger_type() {
        let (engine, worker, _adapter) = setup_queue_worker();
    ...
    #[test]
    fn build_creates_worker() {
        crate::workers::observability::metrics::ensure_default_meter();
        let engine = Arc::new(Engine::new());
        let adapter: Arc<dyn QueueAdapter> = Arc::new(MockQueueAdapter::new());
        let config = super::super::config::QueueWorkerConfig::default();
        let worker = QueueWorker::build(engine.clone(), config, adapter);
        assert_eq!(Worker::name(&worker), "QueueWorker");
    }
    ...
    fn setup_queue_worker_with_configs() -> (Arc<Engine>, QueueWorker, Arc<MockQueueAdapter>) {
    ...
    async fn enqueue_to_function_queue_unknown_queue_fails() {
        let (_engine, worker, _adapter) = setup_queue_worker_with_configs();
        let result = worker
    ...
    async fn enqueue_to_function_queue_fifo_with_group_field_ok() {
        let (_engine, worker, adapter) = setup_queue_worker_with_configs();
        let result = worker
            .enqueue_to_function_queue(
                "payment",
                "fn-1",
    ...
    use crate::workers::module::ConfigurableWorker;

    async fn setup_integration_worker() -> (
        Arc<Engine>,
        QueueWorker,
    ...
    async fn integration_enqueue_consume_invoke_ack() {
        let (engine, worker, adapter) = setup_integration_worker().await;
    ...
    fn resolve_queue_key_function_queue_gets_prefixed() {
        let (_engine, worker, _adapter) = setup_queue_worker_with_configs();
        assert_eq!(worker.resolve_queue_key("default"), "__fn_queue::default");
        assert_eq!(worker.resolve_queue_key("payment"), "__fn_queue::payment");
    }

    #[test]
    fn resolve_queue_key_topic_passes_through() {
        let (_engine, worker, _adapter) = setup_queue_worker_with_configs();
        assert_eq!(worker.resolve_queue_key("some-topic"), "some-topic");
        assert_eq!(
            worker.resolve_queue_key("events.user.created"),
            "events.user.created"
        );
    }
    ...
    fn resolve_queue_key_empty_config_passes_through() {
        let (_engine, worker, _adapter) = setup_queue_worker();
        assert_eq!(worker.resolve_queue_key("anything"), "anything");
    }
    ...
    async fn redrive_message_empty_queue_returns_failure() {
        ...
        let result = worker.redrive_message(input).await;
        match result {
            FunctionResult::Failure(e) => {
                assert_eq!(e.code, "queue_not_set");
            }
    ...
    async fn discard_message_empty_queue_returns_failure() {
        ...
        let result = worker.discard_message(input).await;
        match result {
            FunctionResult::Failure(e) => {
                assert_eq!(e.code, "queue_not_set");
            }
    ...
    async fn console_list_topics_returns_topics() {
        ...
        *adapter.list_topics_result.lock().await = vec![TopicInfo {
        ...
        let result = worker.console_list_topics(json!({})).await;
        match result {
    ...
    async fn console_topic_stats_empty_topic_returns_failure() {
        let (_engine, worker, _adapter) = setup_queue_worker();
        let result = worker.console_topic_stats(json!({})).await;
        match result {
            FunctionResult::Failure(e) => {
    ...
    async fn console_dlq_topics_returns_topics_with_counts() {
        ...
        *adapter.list_topics_result.lock().await = vec![TopicInfo {
        ...
        let result = worker.console_dlq_topics(json!({})).await;
        match result {
            FunctionResult::Success(Some(val)) => {
                let topics: Vec<Value> = serde_json::from_value(val).unwrap();
    ...
    async fn console_dlq_messages_empty_topic_returns_failure() {
        let (_engine, worker, _adapter) = setup_queue_worker();
        let result = worker.console_dlq_messages(json!({})).await;
        match result {
            FunctionResult::Failure(e) => {
                assert_eq!(e.code, "topic_required");
            }
