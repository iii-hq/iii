...
impl Exec {
    pub fn new(config: ExecConfig) -> Self {
        tracing::info!("Creating Exec worker with config: {:?}", config);
    ...
    pub async fn shutdown(&self) {
        ...
        tracing::info!("ExecWorker received shutdown signal, stopping process");
        ...
        loop {
            tokio::select! {
                ...
                _ = shutdown_rx.changed() => {
                    tracing::info!("ExecWorker file watcher shutting down");
                    break;
                }
            }
        }
