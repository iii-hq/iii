...
mod tests {
    ...
    fn build_cors_layer_no_config_returns_permissive() {
        let worker = make_worker_with_cors(None);
        let _cors = worker.build_cors_layer();
    }

    #[test]
    fn build_cors_layer_empty_origins_allows_any() {
        let worker = make_worker_with_cors(Some(CorsConfig {
            allowed_origins: vec![],
            allowed_methods: vec![],
        }));
        let _cors = worker.build_cors_layer();
    ...
    fn build_routers_from_empty_registry() {
        ...
        let _router =
            HttpWorker::build_routers_from_routers_registry(engine, Arc::new(worker), &registry);
    }
    ...
    async fn test_get_router_found() {
        ...
        worker.routers_registry.insert(
        ...
        let result = worker.get_router("GET", "users/:id");
    ...
    async fn test_get_router_not_found() {
        let worker = make_worker_with_cors(None);
        let result = worker.get_router("GET", "nonexistent");
    ...
    async fn test_get_router_normalizes_path() {
        ...
        let result = worker.get_router("POST", "/items");
        assert!(result.is_some());
    ...
    async fn create_name_register_functions_and_initialize_work() {
        ...
        let worker = <HttpWorker as Worker>::create(
            engine.clone(),
            Some(json!({
                "host": "127.0.0.1",
                "port": 0,
        ...
        assert_eq!(worker.name(), "HttpWorker");

        worker.register_functions(engine.clone());
        worker.initialize().await.expect("worker should initialize");
    ...
    async fn register_trigger_and_unregister_trigger_manage_routes() {
        ...
        assert!(worker.get_router("POST", "/users/:id").is_none());
    }
