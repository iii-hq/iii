                ...
                tokio::select! {
                    _ = tokio::time::sleep(tokio::time::Duration::from_secs(duration_secs)) => {
                        ...
                        if new_functions_hash != current_functions_hash {
                            ...
                            let functions = worker_worker.list_functions();
                            ...
                            // Fire triggers directly from this worker
...
mod tests {
    ...
    fn setup_engine_and_worker() -> (Arc<Engine>, EngineFunctionsWorker) {
        ensure_default_meter();
        let engine = Arc::new(Engine::new());
        let worker = EngineFunctionsWorker::new(engine.clone());
        (engine, worker)
    }
    ...
    fn test_list_functions_empty_engine() {
        let (_engine, worker) = setup_engine_and_worker();
    ...
    fn test_list_functions_returns_registered_functions() {
        let (engine, worker) = setup_engine_and_worker();
    ...
    async fn test_list_trigger_infos_empty() {
        let (_engine, worker) = setup_engine_and_worker();
        let triggers = worker.list_trigger_infos().await;
    ...
    async fn test_list_trigger_infos_returns_registered_triggers() {
        ...
        let infos = worker.list_trigger_infos().await;
    ...
    async fn test_list_worker_infos_empty() {
        let (_engine, worker) = setup_engine_and_worker();
        let workers = worker.list_worker_infos(None).await;
    ...
    async fn test_register_worker_metadata_invalid_uuid() {
        ...
        worker.register_worker_metadata(input).await;
    }
    ...
    async fn test_register_worker_metadata_defaults_runtime_to_unknown() {
        ...
        let workers = worker.list_worker_infos(Some(&worker_id)).await;
    ...
    async fn test_trigger_registrator_register_and_unregister() {
        ...
        assert!(worker.triggers.contains_key("eng-trig-1"));

        // Unregister
        let result = worker.unregister_trigger(trigger).await;
        assert!(result.is_ok());
        assert!(worker.triggers.is_empty());
    }
    ...
    async fn test_get_triggers_filters_internal_by_default() {
        ...
        worker.engine.trigger_registry.triggers.insert(
            "internal".to_string(),
