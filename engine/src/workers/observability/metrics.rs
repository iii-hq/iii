...
mod tests {
    ...
    /// Reproduce the shared-store interleaving without touching process globals:
    /// a writer inserts after clear but before ingestion/count. An owned store
    /// must retain exactly the two resources even when that other store changes.
    #[test]
    fn test_ingest_metrics_owned_storage_excludes_interleaved_writer() {
