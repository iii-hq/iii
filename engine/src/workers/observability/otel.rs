...
//!
//! This worker provides OTLP/gRPC trace export and integrates with the existing
...
/// Set the global OTEL configuration from the worker.
/// This should be called during worker initialization, before logging is set up.
///
...
pub fn set_otel_config(config: OtelWorkerConfig) -> bool {
    if GLOBAL_OTEL_CONFIG.set(config).is_ok() {
        ...
        // Config already set - this can happen if worker is re-initialized
