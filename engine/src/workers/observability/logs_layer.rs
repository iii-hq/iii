...
mod tests {
    ...
    fn layer_captures_target_attribute() {
        let (_, logs) = with_layer(|| {
            tracing::info!("message from worker");
        });
        assert_eq!(logs.len(), 1);
    ...
    #[test]
    fn layer_worker_path_attribute() {
        let (_, logs) = with_layer(|| {
            tracing::info!("with worker path");
        });
        assert_eq!(logs.len(), 1);
        assert!(logs[0].attributes.contains_key("worker_path"));
    }
