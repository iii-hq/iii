...
pub trait Worker: Send + Sync {
    ...
    async fn destroy(&self) -> anyhow::Result<()> {
        tracing::info!("Destroying worker: {}", self.name());
        Ok(())
    }
...
pub trait ConfigurableWorker: Worker + Sized + 'static {
    ...
    /// ```
    async fn registry() -> &'static RwLock<HashMap<String, AdapterFactory<Self::Adapter>>>;
    /// Build the worker from parsed config and adapter
    ...
    /// Extract adapter config from worker config (optional override)
...
mod tests {
    ...
    #[tokio::test]
    async fn worker_default_trait_methods_work() {
        let engine = Arc::new(Engine::new());
        let worker = SimpleWorker::make_worker(engine, None)
            .await
            .expect("make worker should succeed");

        assert_eq!(worker.name(), "SimpleWorker");
    ...
    #[tokio::test]
    async fn worker_register_functions_default_is_noop() {
        let engine = Arc::new(Engine::new());
    ...
    #[tokio::test]
    async fn worker_default_trait_methods_on_concrete_type_work() {
        let (tx, rx) = tokio::sync::watch::channel(false);
        let worker = SimpleWorker;
    ...
    #[tokio::test]
    #[serial]
    async fn configurable_worker_helper_defaults_are_accessible_directly() {
        clear_test_registry().await;
    ...
    async fn create_with_adapters_uses_default_and_custom_configured_adapter() {
        ...
        let default_worker = TestConfigurableWorker::create_with_adapters(engine.clone(), None)
            .await
            .expect("create with default adapter");
        assert_eq!(default_worker.name(), "TestConfigurableWorker");
        ...
        let custom_worker = TestConfigurableWorker::create_with_adapters(
            engine,
            Some(serde_json::to_value(configured).expect("serialize test config")),
        )
        .await
        .expect("create with custom adapter");
        assert_eq!(custom_worker.name(), "TestConfigurableWorker");
    ...
    async fn build_registry_and_default_hooks_cover_inventory_path() {
        ...
        let worker = DefaultHooksWorker::create_with_adapters(
            Arc::new(Engine::new()),
            Some(serde_json::json!({ "enabled": true })),
        )
        .await
        .expect("create worker from inventory-backed registry");
        assert_eq!(worker.name(), "DefaultHooksWorker");
        assert_eq!(
