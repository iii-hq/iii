...
mod tests {
    ...
    async fn test_register_join_trigger() {
        ...
        let joins = worker.triggers.join_triggers.read().await;
    ...
    async fn test_register_leave_trigger() {
        ...
        let leaves = worker.triggers.leave_triggers.read().await;
    ...
    async fn test_register_stream_trigger() {
        ...
        let stream_triggers = worker.triggers.stream_triggers.read().await;
        assert_eq!(stream_triggers.len(), 1);
        assert!(stream_triggers.contains_key("stream-1"));

        let by_name = worker.triggers.stream_triggers_by_name.read().await;
        let ids = by_name
            .get("my-stream")
    ...
    async fn test_unregister_one_of_multiple_stream_triggers() {
        ...
        worker.register_trigger(trigger1.clone()).await.unwrap();
        worker.register_trigger(trigger2).await.unwrap();

        // Unregister only trigger1
        worker.unregister_trigger(trigger1).await.unwrap();
