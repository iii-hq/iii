...
async fn ws_handler(
    State(worker): State<Arc<StreamSocketManager>>,
    ws: WebSocketUpgrade,
    uri: Uri,
    headers: HeaderMap,
    ...
    if let Some(auth_function) = worker.auth_function.clone() {
                    ...
                    match context {
                        Ok(context) => {
                            return ws.on_upgrade(move |socket: WebSocket| async move {
                                    if let Err(err) = worker.socket_handler(socket, Some(context)).await {
    ...
    ws.on_upgrade(move |socket: WebSocket| async move {
        if let Err(err) = worker.socket_handler(socket, None).await {
            tracing::error!(addr = %addr, error = ?err, "stream socket error");
        }
    })
}
...
impl Worker for StreamWorker {
    ...
    async fn initialize(&self) -> anyhow::Result<()> {
        ...
        let listener = TcpListener::bind(addr)
            .await
            .map_err(|err| crate::workers::module::bind_address_error(addr, err))?;
        let app = Router::new()
...
mod tests {
    ...
    fn create_test_worker() -> StreamWorker {
    ...
    fn create_worker_with_adapter(adapter: Arc<dyn StreamAdapter>) -> StreamWorker {
        crate::workers::observability::metrics::ensure_default_meter();
        let engine = Arc::new(Engine::new());
        let config = StreamWorkerConfig {
            port: 0,
    ...
    #[tokio::test]
    async fn test_stream_worker_set_get_delete() {
        ...
        // Start event watcher
        let watcher_adapter = worker.adapter.clone();
    ...
    #[tokio::test]
    async fn test_stream_worker_update_existing_record() {
        let worker = create_test_worker();
        let stream_name = "test_stream";
        let group_id = "test_group";
        let item_id = "item1";
    ...
    #[tokio::test]
    async fn test_stream_worker_update_new_record() {
        let worker = create_test_worker();
        let stream_name = "test_stream";
        let group_id = "test_group";
    ...
    async fn test_stream_list_groups_list_all_and_send_with_adapter() {
        ...
        *adapter
        ...
        let worker = create_worker_with_adapter(adapter.clone());
        ...
        match worker.list_all(StreamListAllInput {}).await {
            FunctionResult::Success(result) => {
    ...
    async fn test_stream_methods_return_adapter_failures() {
        ...
        *adapter.update_result.lock().expect("lock update_result") =
        ...
        let worker = create_worker_with_adapter(adapter);
        ...
        assert!(matches!(
            worker.list_all(StreamListAllInput {}).await,
    ...
    async fn test_stream_custom_functions_override_adapter() {
        ...
        worker.engine.register_function_handler(
    ...
    async fn test_stream_invoke_triggers_covers_conditions_and_errors() {
        ...
        let mut stream_triggers = worker.triggers.stream_triggers.write().await;
    ...
    async fn test_stream_initialize_registers_trigger_types_and_destroy_calls_adapter() {
        ...
        worker.destroy().await.expect("stream destroy should work");
