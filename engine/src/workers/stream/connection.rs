...
pub struct SocketStreamConnection {
    ...
    stream_worker: Arc<StreamWorker>,
...
mod tests {
    ...
    fn build_connection(
        ...
        let worker = Arc::new(StreamWorker::build(
            engine.clone(),
    ...
    async fn handle_join_leave_invokes_join_and_leave_triggers() {
        ...
        let (engine, _worker, connection, _rx) = build_connection(Some(context));
    ...
    async fn handle_socket_message_unauthorized_join_emits_unauthorized() {
        let (engine, _worker, connection, mut rx) = build_connection(None);
    ...
    async fn handle_socket_message_join_with_id_syncs_get_and_leave_removes_subscription() {
        let (_engine, worker, connection, mut rx) = build_connection(None);
    ...
    async fn handle_socket_message_join_without_id_syncs_list_and_cleanup_emits_leave_events() {
        let (engine, worker, connection, mut rx) = build_connection(None);
    ...
    async fn handle_join_leave_ignores_trigger_errors() {
        let (engine, _worker, connection, _rx) = build_connection(None);
