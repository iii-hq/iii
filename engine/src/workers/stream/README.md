...
`iii-stream` is engine-owned. Configure it in the top-level `workers` list in `config.yaml`.
...
restarting the engine. The `workers` list in `config.yaml` is the **seed** used on first boot
