...
function ConfigPage() {
  ...
  const { data: adaptersData, refetch: refetchAdapters, isLoading: isLoadingAdapters } = useQuery(adaptersQuery)
  const { data: statusData, refetch: refetchStatus, isLoading: isLoadingStatus } = useQuery(statusQuery)
        ...
        <div className="flex-1 p-4 md:p-6 space-y-6">
          <div className="grid grid-cols-6 gap-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="p-3 rounded-[var(--radius-lg)] bg-elevated border border-border-subtle">
      ...
      <div className="grid grid-cols-6 gap-3 p-4 md:p-6 border-b border-border bg-dark-gray/20">
        <div className="p-3 rounded-[var(--radius-lg)] bg-elevated border border-border-subtle">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Modules</span>
          </div>
          <div className="font-mono text-[13px] text-2xl font-bold text-cyan">{stats.modules}</div>
        </div>
        <div className="p-3 rounded-[var(--radius-lg)] bg-elevated border border-border-subtle">
          <div className="flex items-center justify-between mb-1">
            <Box className="w-4 h-4 text-purple-400" />
            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Functions</span>
          </div>
          <div className="font-mono text-[13px] text-2xl font-bold text-purple-400">{stats.functions}</div>
        </div>
        <div className="p-3 rounded-[var(--radius-lg)] bg-elevated border border-border-subtle">
          <div className="flex items-center justify-between mb-1">
            <Zap className="w-4 h-4 text-yellow" />
            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Triggers</span>
          </div>
          <div className="font-mono text-[13px] text-2xl font-bold text-yellow">{stats.triggers}</div>
        </div>
        <div className="p-3 rounded-[var(--radius-lg)] bg-elevated border border-border-subtle">
          <div className="flex items-center justify-between mb-1">
            <Database className="w-4 h-4 text-green-400" />
            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Streams</span>
          </div>
          <div className="font-mono text-[13px] text-2xl font-bold text-green-400">{stats.streams}</div>
        </div>
        <div className="p-3 rounded-[var(--radius-lg)] bg-elevated border border-border-subtle">
          <div className="flex items-center justify-between mb-1">
            <Users className="w-4 h-4 text-blue-400" />
            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Workers</span>
          </div>
          <div className="font-mono text-[13px] text-2xl font-bold text-blue-400">{stats.workers}</div>
        </div>
        <div className="p-3 rounded-[var(--radius-lg)] bg-elevated border border-border-subtle">
          <div className="flex items-center justify-between mb-1">
            <CheckCircle className="w-4 h-4 text-success" />
            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Healthy</span>
          </div>
          <div className="font-mono text-[13px] text-2xl font-bold text-success">{stats.healthy}</div>
        </div>
      </div>
                  ...
                  <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-4">
                    {triggerHandlers.map((handler) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <Zap className="w-3.5 h-3.5 text-yellow" />
                          <span className="font-sans font-medium text-sm truncate">{handler.id}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-[10px] px-1.5 py-0.5 rounded ${
                  ...
                  <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                    {workers.map((worker) => (
                      ...
                      >
                        ...
                        <div className="font-sans text-[10px] text-secondary">connected workers</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {selectedModuleData && (
          <div className="border-l border-border bg-elevated overflow-y-auto">
            <div className="p-4 border-b border-border sticky top-0 bg-elevated/90 backdrop-blur">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-sans font-medium text-sm">{selectedModuleData.id.split('::').pop()}</h2>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
            ...
            <div className="p-4 space-y-4">
              <div>
                <div className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted mb-2">Full ID</div>
              ...
              <div>
                <div className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted mb-2">Type</div>
                <Badge variant="outline">{selectedModuleData.type}</Badge>
              </div>

              <div>
                <div className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted mb-2">Status</div>
                <div className="flex items-center gap-2">
                  <span
              ...
              {selectedModuleData.port && (
                <div>
                  <div className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted mb-2">Port</div>
              ...
              {selectedModuleData.description && (
                <div>
                  ...
                  <p className="font-sans text-sm text-secondary">{selectedModuleData.description}</p>
                </div>
              )}
