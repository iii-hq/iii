...
export function QueueDlqTab({ topic, dlqEntry }: QueueDlqTabProps) {
  ...
  const messageRedriveMutation = useMutation({
    ...
    onMutate: async (messageId) => {
      ...
      const prev = queryClient.getQueryData<{ topic: string; messages: DlqMessage[] }>(['dlq-messages', topic, 0, 50])
      if (prev) {
      ...
      <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-dark-gray/30">
        ...
        {confirmRedriveAll ? (
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-xs text-muted font-mono">Redrive all {messages.length} messages?</span>
            <Button
              variant="accent"
              size="sm"
              onClick={() => {
          ...
          >
            <RotateCcw className={`w-3 h-3 ${bulkRedriveMutation.isPending ? 'animate-spin' : ''}`} />
            Redrive All
          </Button>
        )}
      </div>
                ...
                return (
                  <div key={m.id} className="group/row">
                    ...
                    <div className={`relative flex items-center transition-colors ${
                      ...
                      >
                        <ChevronRight className={`w-3.5 h-3.5 shrink-0 text-muted transition-transform ${isExpanded ? 'rotate-90' : ''}`} />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-[13px] text-foreground truncate">{m.id}</span>
                            <Badge variant={m.retries > 2 ? 'warning' : 'default'} className="text-[10px] shrink-0">
                              {m.retries}x
                            </Badge>
                            <span className="font-mono text-[11px] text-muted shrink-0">{formatBytes(m.size_bytes)}</span>
                          </div>
                      ...
                      <div className="flex items-center gap-1 pr-3 opacity-0 group-hover/row:opacity-100 transition-opacity shrink-0">
                        <Tooltip label="Redrive">
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); messageRedriveMutation.mutate(m.id) }}
                        ...
                        {confirmDiscard === m.id ? (
                          <div className="flex items-center gap-1 bg-error/10 border border-error/20 rounded-[var(--radius-md)] px-2 py-0.5">
                            ...
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); discardMutation.mutate(m.id); setConfirmDiscard(null) }}
                          ...
                          <Tooltip label="Discard">
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); setConfirmDiscard(m.id) }}
                      ...
                      <div className="bg-dark-gray/10 border-l-2 border-l-error px-5 py-4 space-y-4">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            className="h-7 text-xs gap-1.5"
                          >
                            <RotateCcw className={`w-3 h-3 ${messageRedriveMutation.isPending ? 'animate-spin' : ''}`} />
                            Redrive
                          </Button>
                          {confirmDiscard === m.id ? (
                            <>
                          ...
                          <span className="ml-auto font-mono text-[11px] text-muted">
                            {new Date(expandedMsg.failed_at * 1000).toLocaleString()} · {formatBytes(expandedMsg.size_bytes)}
                          </span>
                        </div>
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 mb-1.5">
                            ...
                            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Error</span>
                          </div>
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 mb-1.5">
                            ...
                            <span className="font-sans font-semibold text-xs uppercase tracking-[0.04em] text-muted">Payload</span>
                          </div>
